<template>

    <div class="h-[30px]"></div>

    <section class="w-full pb-16">

        <div class="mx-auto lg:max-w-[1000px] xl:max-w-[1400px] py-2 px-5 sm:px-7 sm:py-0 flex flex-wrap">

            <div class="w-full sm:w-1/3 mb-5">
                <img :src="'/images/image-27.png'" class="object-cover object-top w-full h-[350px]" alt="agent">
            </div>

            <div class="w-full sm:w-2/3 sm:ps-10 mb-5">

                <div class="text-[25px] mb-1"> Team details </div>

                <div class="text-[18px] mb-2">
                    Samuel Palmer
                </div>

                <div class="flex justify-start items-center flex-wrap gap-3">
                    5 🌟🌟🌟🌟🌟 <span> See all reviews </span>
                </div>

                <div class="mt-2 flex justify-start items-center flex-wrap text-[13px]">
                    Company Agent at <router-link :to="{name: 'Properties'}" class="mx-2 text-blue-600"> Modern House Real Estate </router-link>
                </div>

                <div class="my-2 w-full border border-gray-300"></div>

                <div class="mt-3 text-[13px]">
                    <span class="font-bold me-2"> Agent License: </span>
                    090-0348-8346
                </div>

                <div class="mt-3 text-[13px]">
                    <span class="font-bold me-2"> Tax Number: </span>
                    HGT-92384-3434
                </div>

                <div class="mt-3 text-[13px]">
                    <span class="font-bold me-2"> Service Areas: </span>
                    Chicago, Los Angeles, Miami Beach, New York
                </div>

                <div class="mt-3 text-[13px]">
                    <span class="font-bold me-2"> Specialties: </span>
                    Property management, Real estate development, Real estate appraising, Retail leasing, Apartment brokerage
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 text-[13px] gap-3 my-5 w-full sm:w-[290px]">

                    <a href="javascript:void(0)" class="bg-blue-600 text-center decoration-0 text-white duration-500 hover:bg-blue-950 px-4 rounded py-2 whitespace-break-spaces">
                        Send Message
                    </a>

                    <a href="javascript:void(0)" class="border border-blue-600 text-blue-600 text-center decoration-0 hover:border-blue-950 hover:text-white duration-500 hover:bg-blue-950 px-4 rounded py-2 whitespace-break-spaces">
                        Call
                    </a>

                </div>

            </div>

            <div class="w-full sm:w-2/3 mb-5">

                <div class="text-[25px] mb-2"> About Samuel Palmer </div>

                <div class="w-[65px] border border-gray-400 mb-3"></div>

                <div class="text-gray-600 my-2 text-[14px]">
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus porta justo eget risus consectetur, non venenatis elit blandit. Mauris vehicula, libero a iaculis fringilla, ipsum ipsum tincidunt velit, ut convallis velit ante tincidunt dui. Sed iaculis ullamcorper pellentesque. Nam congue nisi eu orci laoreet, nec tristique dolor scelerisque. Aenean mauris sem, commodo et accumsan ac, dictum vitae sem. Sed bibendum nunc neque, in auctor enim ultricies nec. Proin ornare nibh libero, id euismod nulla aliquam et. Nam eget augue ut dolor sagittis feugiat. Nullam et nibh id lacus mollis laoreet eu et mi.
                </div>

            </div>

            <div class="w-full sm:w-1/3 mb-5 sm:ps-10">

                <div class="w-full grid grid-cols-2 gap-5">
                    <div class="text-start font-medium">
                        Office
                    </div>
                    <div class="text-end text-gray-600 text-[13px]">
                        321 456 9865
                    </div>
                    <div class="text-start font-medium">
                        Mobile
                    </div>
                    <div class="text-end text-gray-600 text-[13px]">
                        321 456 9874
                    </div>
                    <div class="text-start font-medium">
                        Fax
                    </div>
                    <div class="text-end text-gray-600 text-[13px]">
                        897 654 5487
                    </div>
                    <div class="text-start font-medium">
                        Email
                    </div>
                    <div class="text-end text-gray-600 text-[13px]">
                        samuel@houzez.com
                    </div>
                    <div class="text-start font-medium">
                        Website
                    </div>
                    <div class="text-end text-gray-600 text-[13px]">
                        https://houzez.co
                    </div>
                </div>
            </div>

            <div class="mb-5 text-[25px]">My list of contract</div>

            <div class="w-full flex flex-wrap">

                <div class="w-full lg:w-2/3 order-2 lg:order-1">

                    <!-- Property card -->
                    <div class="decoration-0 w-full">
                        <div class="mb-5 border flex flex-wrap duration-500 hover:shadow-xl group">
                            <div class="w-full sm:w-1/2">

                                <div class="relative">

                                    <!-- Property card image -->
                                    <img :src="'/images/properties-1.jpg'" class="object-cover bg-cover w-full h-[250px]" alt="properties"/>

                                    <div class="absolute bottom-0 start-0 top-0 end-0 p-3 flex justify-center items-center gap-x-3 bg-blue-200 bg-opacity-45 scale-0 duration-500 group-hover:scale-100">
                                        <button type="button" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill="#ffffff" d="M16.5562 12.9062L16.1007 13.359C16.1007 13.359 15.0181 14.4355 12.0631 11.4972C9.10812 8.55901 10.1907 7.48257 10.1907 7.48257L10.4775 7.19738C11.1841 6.49484 11.2507 5.36691 10.6342 4.54348L9.37326 2.85908C8.61028 1.83992 7.13596 1.70529 6.26145 2.57483L4.69185 4.13552C4.25823 4.56668 3.96765 5.12559 4.00289 5.74561C4.09304 7.33182 4.81071 10.7447 8.81536 14.7266C13.0621 18.9492 17.0468 19.117 18.6763 18.9651C19.1917 18.9171 19.6399 18.6546 20.0011 18.2954L21.4217 16.883C22.3806 15.9295 22.1102 14.2949 20.8833 13.628L18.9728 12.5894C18.1672 12.1515 17.1858 12.2801 16.5562 12.9062Z"></path>
                                                </g>
                                            </svg>
                                        </button>
                                        <button type="button" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="stroke-white duration-500 group-hover/item:stroke-blue-600" d="M4 7.00005L10.2 11.65C11.2667 12.45 12.7333 12.45 13.8 11.65L20 7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                    <rect class="stroke-white duration-500 group-hover/item:stroke-blue-600" x="3" y="5" width="18" height="14" rx="2" stroke-width="2" stroke-linecap="round"></rect>
                                                </g>
                                            </svg>
                                        </button>
                                        <router-link :to="{name: 'PropertyDetails'}" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M11.9944 15.5C13.9274 15.5 15.4944 13.933 15.4944 12C15.4944 10.067 13.9274 8.5 11.9944 8.5C10.0614 8.5 8.49439 10.067 8.49439 12C8.49439 13.933 10.0614 15.5 11.9944 15.5ZM11.9944 13.4944C11.1691 13.4944 10.5 12.8253 10.5 12C10.5 11.1747 11.1691 10.5056 11.9944 10.5056C12.8197 10.5056 13.4888 11.1747 13.4888 12C13.4888 12.8253 12.8197 13.4944 11.9944 13.4944Z"></path>
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M12 5C7.18879 5 3.9167 7.60905 2.1893 9.47978C0.857392 10.9222 0.857393 13.0778 2.1893 14.5202C3.9167 16.391 7.18879 19 12 19C16.8112 19 20.0833 16.391 21.8107 14.5202C23.1426 13.0778 23.1426 10.9222 21.8107 9.47978C20.0833 7.60905 16.8112 5 12 5ZM3.65868 10.8366C5.18832 9.18002 7.9669 7 12 7C16.0331 7 18.8117 9.18002 20.3413 10.8366C20.9657 11.5128 20.9657 12.4872 20.3413 13.1634C18.8117 14.82 16.0331 17 12 17C7.9669 17 5.18832 14.82 3.65868 13.1634C3.03426 12.4872 3.03426 11.5128 3.65868 10.8366Z"></path>
                                                </g>
                                            </svg>
                                        </router-link>
                                    </div>

                                </div>

                            </div>
                            <div class="w-full sm:w-1/2 p-3 py-3 flex justify-between flex-col">
                                <div class="mb-3">
                                    <div class="mb-3 text-[21px]">

                                        <!-- Property card title -->
                                        <div class="truncate-to-1-line">
                                            Modern Apartment
                                        </div>
                                    </div>
                                    <div class="text-[13px]">

                                        <!-- Property card description -->
                                        <div class="text-gray-500">
                                            4936 N Broadway St, Chicago, IL 60640, USA
                                        </div>

                                    </div>

                                    <div class="flex justify-start items-center text-[15px] gap-x-3 mt-3">

                                        <!-- bed count -->
                                        <div class="flex justify-start items-center">

                                            <svg class="fill-black size-[15px]" viewBox="0 0 50 50" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path d="M5 10C3.355469 10 2 11.355469 2 13L2 28.1875C2.003906 28.25 2.015625 28.3125 2.03125 28.375C2.03125 28.386719 2.03125 28.394531 2.03125 28.40625C1.582031 29.113281 1.214844 29.867188 0.9375 30.6875C0.316406 32.519531 0.0507813 34.621094 0 37L0 38C0 38.03125 0 38.0625 0 38.09375L0 50L7 50L7 46C7 45.167969 7.203125 44.734375 7.46875 44.46875C7.734375 44.203125 8.167969 44 9 44L41 44C41.832031 44 42.265625 44.203125 42.53125 44.46875C42.796875 44.734375 43 45.167969 43 46L43 50L50 50L50 38.15625C50.003906 38.105469 50.003906 38.050781 50 38C50 37.65625 50.007813 37.332031 50 37C49.949219 34.621094 49.683594 32.519531 49.0625 30.6875C48.785156 29.875 48.414063 29.136719 47.96875 28.4375C47.988281 28.355469 48 28.273438 48 28.1875L48 13C48 11.355469 46.644531 10 45 10 Z M 5 12L45 12C45.5625 12 46 12.4375 46 13L46 26.15625C45.753906 25.949219 45.492188 25.75 45.21875 25.5625C44.550781 25.101563 43.824219 24.671875 43 24.3125L43 20C43 19.296875 42.539063 18.75 42.03125 18.40625C41.523438 18.0625 40.902344 17.824219 40.125 17.625C38.570313 17.226563 36.386719 17 33.5 17C30.613281 17 28.429688 17.226563 26.875 17.625C26.117188 17.820313 25.5 18.042969 25 18.375C24.5 18.042969 23.882813 17.820313 23.125 17.625C21.570313 17.226563 19.386719 17 16.5 17C13.613281 17 11.429688 17.226563 9.875 17.625C9.097656 17.824219 8.476563 18.0625 7.96875 18.40625C7.460938 18.75 7 19.296875 7 20L7 24.3125C6.175781 24.671875 5.449219 25.101563 4.78125 25.5625C4.507813 25.75 4.246094 25.949219 4 26.15625L4 13C4 12.4375 4.4375 12 5 12 Z M 16.5 19C19.28125 19 21.34375 19.234375 22.625 19.5625C23.265625 19.726563 23.707031 19.925781 23.90625 20.0625C23.988281 20.117188 23.992188 20.125 24 20.125L24 22C17.425781 22.042969 12.558594 22.535156 9 23.625L9 20.125C9.007813 20.125 9.011719 20.117188 9.09375 20.0625C9.292969 19.925781 9.734375 19.726563 10.375 19.5625C11.65625 19.234375 13.71875 19 16.5 19 Z M 33.5 19C36.28125 19 38.34375 19.234375 39.625 19.5625C40.265625 19.726563 40.707031 19.925781 40.90625 20.0625C40.988281 20.117188 40.992188 20.125 41 20.125L41 23.625C37.441406 22.535156 32.574219 22.042969 26 22L26 20.125C26.007813 20.125 26.011719 20.117188 26.09375 20.0625C26.292969 19.925781 26.734375 19.726563 27.375 19.5625C28.65625 19.234375 30.71875 19 33.5 19 Z M 24.8125 24C24.917969 24.015625 25.019531 24.015625 25.125 24C25.15625 24 25.1875 24 25.21875 24C35.226563 24.015625 41.007813 25.0625 44.09375 27.1875C45.648438 28.257813 46.589844 29.585938 47.1875 31.34375C47.707031 32.875 47.917969 34.761719 47.96875 37L2.03125 37C2.082031 34.761719 2.292969 32.875 2.8125 31.34375C3.410156 29.585938 4.351563 28.257813 5.90625 27.1875C8.992188 25.058594 14.785156 24.011719 24.8125 24 Z M 2 39L48 39L48 48L45 48L45 46C45 44.832031 44.703125 43.765625 43.96875 43.03125C43.234375 42.296875 42.167969 42 41 42L9 42C7.832031 42 6.765625 42.296875 6.03125 43.03125C5.296875 43.765625 5 44.832031 5 46L5 48L2 48Z"></path>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 2 </span>
                                        </div>

                                        <!-- bath count -->
                                        <div class="flex justify-start items-center">

                                            <svg viewBox="0 0 45 45" class="size-[15px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="stroke-black" d="M27.7812 11.002V9.002C27.7812 7.897 26.8863 7.002 25.7812 7.002H14.7812C13.6762 7.002 12.7812 7.897 12.7812 9.002V36.002" stroke-width="2"></path>
                                                    <path class="stroke-black" d="M33.7812 11.002H21.7812V15.002H33.7812V11.002Z" stroke-width="2" stroke-linejoin="round"></path>
                                                    <path class="stroke-black" d="M18.7812 15.002H36.7812" stroke-width="2" stroke-linejoin="round"></path>
                                                    <path class="fill-black" d="M27.7812 18.002C27.2292 18.002 26.7812 18.45 26.7812 19.002C26.7812 19.554 27.2292 20.002 27.7812 20.002C28.3333 20.002 28.7812 19.554 28.7812 19.002C28.7812 18.45 28.3333 18.002 27.7812 18.002ZM22.7812 18.002C22.2293 18.002 21.7812 18.45 21.7812 19.002C21.7812 19.554 22.2293 20.002 22.7812 20.002C23.3333 20.002 23.7812 19.554 23.7812 19.002C23.7812 18.45 23.3333 18.002 22.7812 18.002ZM32.7812 18.002C32.2292 18.002 31.7812 18.45 31.7812 19.002C31.7812 19.554 32.2292 20.002 32.7812 20.002C33.3333 20.002 33.7812 19.554 33.7812 19.002C33.7812 18.45 33.3333 18.002 32.7812 18.002ZM27.7812 22.002C27.2292 22.002 26.7812 22.45 26.7812 23.002C26.7812 23.554 27.2292 24.002 27.7812 24.002C28.3333 24.002 28.7812 23.554 28.7812 23.002C28.7812 22.45 28.3333 22.002 27.7812 22.002ZM32.7812 22.002C32.2292 22.002 31.7812 22.45 31.7812 23.002C31.7812 23.554 32.2292 24.002 32.7812 24.002C33.3333 24.002 33.7812 23.554 33.7812 23.002C33.7812 22.45 33.3333 22.002 32.7812 22.002ZM22.7812 22.002C22.2293 22.002 21.7812 22.45 21.7812 23.002C21.7812 23.554 22.2293 24.002 22.7812 24.002C23.3333 24.002 23.7812 23.554 23.7812 23.002C23.7812 22.45 23.3333 22.002 22.7812 22.002ZM27.7812 26.002C27.2292 26.002 26.7812 26.45 26.7812 27.002C26.7812 27.554 27.2292 28.002 27.7812 28.002C28.3333 28.002 28.7812 27.554 28.7812 27.002C28.7812 26.45 28.3333 26.002 27.7812 26.002ZM32.7812 26.002C32.2292 26.002 31.7812 26.45 31.7812 27.002C31.7812 27.554 32.2292 28.002 32.7812 28.002C33.3333 28.002 33.7812 27.554 33.7812 27.002C33.7812 26.45 33.3333 26.002 32.7812 26.002ZM22.7812 26.002C22.2293 26.002 21.7812 26.45 21.7812 27.002C21.7812 27.554 22.2293 28.002 22.7812 28.002C23.3333 28.002 23.7812 27.554 23.7812 27.002C23.7812 26.45 23.3333 26.002 22.7812 26.002ZM27.7812 30.002C27.2292 30.002 26.7812 30.45 26.7812 31.002C26.7812 31.554 27.2292 32.002 27.7812 32.002C28.3333 32.002 28.7812 31.554 28.7812 31.002C28.7812 30.45 28.3333 30.002 27.7812 30.002ZM32.7812 30.002C32.2292 30.002 31.7812 30.45 31.7812 31.002C31.7812 31.554 32.2292 32.002 32.7812 32.002C33.3333 32.002 33.7812 31.554 33.7812 31.002C33.7812 30.45 33.3333 30.002 32.7812 30.002ZM22.7812 30.002C22.2293 30.002 21.7812 30.45 21.7812 31.002C21.7812 31.554 22.2293 32.002 22.7812 32.002C23.3333 32.002 23.7812 31.554 23.7812 31.002C23.7812 30.45 23.3333 30.002 22.7812 30.002ZM27.7812 34.002C27.2292 34.002 26.7812 34.45 26.7812 35.002C26.7812 35.554 27.2292 36.002 27.7812 36.002C28.3333 36.002 28.7812 35.554 28.7812 35.002C28.7812 34.45 28.3333 34.002 27.7812 34.002ZM32.7812 34.002C32.2292 34.002 31.7812 34.45 31.7812 35.002C31.7812 35.554 32.2292 36.002 32.7812 36.002C33.3333 36.002 33.7812 35.554 33.7812 35.002C33.7812 34.45 33.3333 34.002 32.7812 34.002ZM22.7812 34.002C22.2293 34.002 21.7812 34.45 21.7812 35.002C21.7812 35.554 22.2293 36.002 22.7812 36.002C23.3333 36.002 23.7812 35.554 23.7812 35.002C23.7812 34.45 23.3333 34.002 22.7812 34.002Z" fill="#3C3C3C"></path> <path d="M32.7812 18.002C32.2292 18.002 31.7812 18.45 31.7812 19.002C31.7812 19.554 32.2292 20.002 32.7812 20.002C33.3333 20.002 33.7812 19.554 33.7812 19.002C33.7812 18.45 33.3333 18.002 32.7812 18.002ZM27.7812 18.002C27.2293 18.002 26.7812 18.45 26.7812 19.002C26.7812 19.554 27.2293 20.002 27.7812 20.002C28.3333 20.002 28.7812 19.554 28.7812 19.002C28.7812 18.45 28.3333 18.002 27.7812 18.002ZM32.7812 22.002C32.2292 22.002 31.7812 22.45 31.7812 23.002C31.7812 23.554 32.2292 24.002 32.7812 24.002C33.3333 24.002 33.7812 23.554 33.7812 23.002C33.7812 22.45 33.3333 22.002 32.7812 22.002ZM27.7812 22.002C27.2293 22.002 26.7812 22.45 26.7812 23.002C26.7812 23.554 27.2293 24.002 27.7812 24.002C28.3333 24.002 28.7812 23.554 28.7812 23.002C28.7812 22.45 28.3333 22.002 27.7812 22.002ZM32.7812 26.002C32.2292 26.002 31.7812 26.45 31.7812 27.002C31.7812 27.554 32.2292 28.002 32.7812 28.002C33.3333 28.002 33.7812 27.554 33.7812 27.002C33.7812 26.45 33.3333 26.002 32.7812 26.002ZM27.7812 26.002C27.2293 26.002 26.7812 26.45 26.7812 27.002C26.7812 27.554 27.2293 28.002 27.7812 28.002C28.3333 28.002 28.7812 27.554 28.7812 27.002C28.7812 26.45 28.3333 26.002 27.7812 26.002ZM32.7812 30.002C32.2292 30.002 31.7812 30.45 31.7812 31.002C31.7812 31.554 32.2292 32.002 32.7812 32.002C33.3333 32.002 33.7812 31.554 33.7812 31.002C33.7812 30.45 33.3333 30.002 32.7812 30.002ZM27.7812 30.002C27.2293 30.002 26.7812 30.45 26.7812 31.002C26.7812 31.554 27.2293 32.002 27.7812 32.002C28.3333 32.002 28.7812 31.554 28.7812 31.002C28.7812 30.45 28.3333 30.002 27.7812 30.002ZM32.7812 34.002C32.2292 34.002 31.7812 34.45 31.7812 35.002C31.7812 35.554 32.2292 36.002 32.7812 36.002C33.3333 36.002 33.7812 35.554 33.7812 35.002C33.7812 34.45 33.3333 34.002 32.7812 34.002ZM27.7812 34.002C27.2293 34.002 26.7812 34.45 26.7812 35.002C26.7812 35.554 27.2293 36.002 27.7812 36.002C28.3333 36.002 28.7812 35.554 28.7812 35.002C28.7812 34.45 28.3333 34.002 27.7812 34.002Z"></path>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 2 </span>
                                        </div>

                                        <!-- sq ft count -->
                                        <div class="flex justify-start items-center">

                                            <svg class="fill-black size-[15px]" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 297 297" xml:space="preserve">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <g>
                                                        <path d="M285.491,1.703c-4.467-1.284-9.238,0.551-11.699,4.49L101.994,281.069c-2.006,3.209-2.112,7.255-0.276,10.565 c1.834,3.311,5.321,5.365,9.105,5.365h171.798c5.75,0,10.412-4.662,10.412-10.412V11.712 C293.033,7.067,289.956,2.983,285.491,1.703z M272.209,276.176h-13.535v-6.768c0-5.75-4.662-10.412-10.412-10.412 s-10.412,4.662-10.412,10.412v6.768h-24.988v-6.768c0-5.75-4.662-10.412-10.412-10.412s-10.412,4.662-10.412,10.412v6.768h-24.989 v-6.768c0-5.75-4.661-10.412-10.411-10.412s-10.412,4.662-10.412,10.412v6.768H129.61L272.209,48.017V276.176z"></path>
                                                        <path d="M193.635,243.53h40.535c5.75,0,10.412-4.661,10.412-10.411V168.26c0-4.645-3.077-8.729-7.542-10.009 c-4.469-1.284-9.237,0.552-11.7,4.49l-40.535,64.859c-2.005,3.21-2.112,7.255-0.276,10.565 C186.362,241.477,189.849,243.53,193.635,243.53z M223.758,204.565v18.142H212.42L223.758,204.565z"></path>
                                                        <path d="M217.984,44.632c3.024-4.89,1.517-11.303-3.37-14.331l-46.387-28.74c-2.349-1.453-5.18-1.916-7.866-1.285 c-2.689,0.633-5.018,2.308-6.472,4.656l-4.371,7.063c-20.907-10.192-44.484-12.863-67.43-7.462 c-26.352,6.201-48.715,22.299-62.967,45.328C-8.446,94.412,2.747,152.213,43.364,183.531l-4.377,7.074 c-3.024,4.888-1.517,11.302,3.369,14.33l46.386,28.745c1.66,1.029,3.562,1.563,5.484,1.563c0.796,0,1.596-0.092,2.384-0.277 c2.689-0.632,5.017-2.307,6.471-4.656L217.984,44.632z M36.83,60.82c11.324-18.299,29.092-31.09,50.03-36.018 c17.521-4.122,35.5-2.303,51.618,5.033l-16.061,25.953c-9.116-3.315-19.003-3.928-28.668-1.651 C80.637,57.222,69.512,65.231,62.418,76.69c-12.755,20.615-8.822,46.932,8.05,63.046L54.41,165.684 C23.526,140.51,15.311,95.594,36.83,60.82z M81.853,121.338c-7.299-9.497-8.393-22.919-1.727-33.689 c4.163-6.728,10.695-11.431,18.393-13.241c4.179-0.984,8.43-1.043,12.52-0.23L81.853,121.338z M90.854,210.492l-28.684-17.775 L166.116,24.749l28.685,17.773l-8.193,13.241l-6.606-4.094c-4.889-3.03-11.306-1.522-14.334,3.365 c-3.029,4.888-1.523,11.306,3.365,14.335l6.617,4.101l-14.576,23.553l-6.607-4.093c-4.89-3.027-11.307-1.521-14.335,3.367 c-3.028,4.888-1.522,11.306,3.367,14.335l6.616,4.1l-14.574,23.553l-6.607-4.094c-4.889-3.026-11.307-1.52-14.335,3.368 c-3.027,4.889-1.521,11.306,3.368,14.334l6.616,4.1l-14.576,23.552l-6.604-4.094c-4.889-3.03-11.307-1.523-14.336,3.364 c-3.029,4.888-1.523,11.306,3.364,14.336l6.617,4.102L90.854,210.492z"></path>
                                                    </g>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 1560 Sq Ft </span>

                                        </div>

                                    </div>

                                </div>
                                <div>

                                    <!-- Property card on sale -->
                                    <div class="mb-3">
                                        <div class="mb-1 text-[13px] text-blue-900"> On sale </div>
                                        <div class="text-[18px]"> $540,000 </div>
                                    </div>

                                    <!-- Property card on author -->
                                    <div>
                                        <div class="text-[13px] text-blue-900"> By </div>
                                        <div class="text-[18px]"> Maria Villalobos </div>
                                    </div>

                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Property card -->
                    <div class="decoration-0 w-full">
                        <div class="mb-5 border flex flex-wrap duration-500 hover:shadow-xl group">
                            <div class="w-full sm:w-1/2">

                                <div class="relative">

                                    <!-- Property card image -->
                                    <img :src="'/images/properties-2.jpg'" class="object-cover bg-cover w-full h-[250px]" alt="properties"/>

                                    <div class="absolute bottom-0 start-0 top-0 end-0 p-3 flex justify-center items-center gap-x-3 bg-blue-200 bg-opacity-45 scale-0 duration-500 group-hover:scale-100">
                                        <button type="button" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill="#ffffff" d="M16.5562 12.9062L16.1007 13.359C16.1007 13.359 15.0181 14.4355 12.0631 11.4972C9.10812 8.55901 10.1907 7.48257 10.1907 7.48257L10.4775 7.19738C11.1841 6.49484 11.2507 5.36691 10.6342 4.54348L9.37326 2.85908C8.61028 1.83992 7.13596 1.70529 6.26145 2.57483L4.69185 4.13552C4.25823 4.56668 3.96765 5.12559 4.00289 5.74561C4.09304 7.33182 4.81071 10.7447 8.81536 14.7266C13.0621 18.9492 17.0468 19.117 18.6763 18.9651C19.1917 18.9171 19.6399 18.6546 20.0011 18.2954L21.4217 16.883C22.3806 15.9295 22.1102 14.2949 20.8833 13.628L18.9728 12.5894C18.1672 12.1515 17.1858 12.2801 16.5562 12.9062Z"></path>
                                                </g>
                                            </svg>
                                        </button>
                                        <button type="button" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="stroke-white duration-500 group-hover/item:stroke-blue-600" d="M4 7.00005L10.2 11.65C11.2667 12.45 12.7333 12.45 13.8 11.65L20 7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                    <rect class="stroke-white duration-500 group-hover/item:stroke-blue-600" x="3" y="5" width="18" height="14" rx="2" stroke-width="2" stroke-linecap="round"></rect>
                                                </g>
                                            </svg>
                                        </button>
                                        <router-link :to="{name: 'PropertyDetails'}" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M11.9944 15.5C13.9274 15.5 15.4944 13.933 15.4944 12C15.4944 10.067 13.9274 8.5 11.9944 8.5C10.0614 8.5 8.49439 10.067 8.49439 12C8.49439 13.933 10.0614 15.5 11.9944 15.5ZM11.9944 13.4944C11.1691 13.4944 10.5 12.8253 10.5 12C10.5 11.1747 11.1691 10.5056 11.9944 10.5056C12.8197 10.5056 13.4888 11.1747 13.4888 12C13.4888 12.8253 12.8197 13.4944 11.9944 13.4944Z"></path>
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M12 5C7.18879 5 3.9167 7.60905 2.1893 9.47978C0.857392 10.9222 0.857393 13.0778 2.1893 14.5202C3.9167 16.391 7.18879 19 12 19C16.8112 19 20.0833 16.391 21.8107 14.5202C23.1426 13.0778 23.1426 10.9222 21.8107 9.47978C20.0833 7.60905 16.8112 5 12 5ZM3.65868 10.8366C5.18832 9.18002 7.9669 7 12 7C16.0331 7 18.8117 9.18002 20.3413 10.8366C20.9657 11.5128 20.9657 12.4872 20.3413 13.1634C18.8117 14.82 16.0331 17 12 17C7.9669 17 5.18832 14.82 3.65868 13.1634C3.03426 12.4872 3.03426 11.5128 3.65868 10.8366Z"></path>
                                                </g>
                                            </svg>
                                        </router-link>
                                    </div>

                                </div>

                            </div>
                            <div class="w-full sm:w-1/2 p-3 py-3 flex justify-between flex-col">
                                <div class="mb-3">
                                    <div class="mb-3 text-[21px]">

                                        <!-- Property card title -->
                                        <div class="truncate-to-1-line">
                                            Modern Apartment
                                        </div>
                                    </div>
                                    <div class="text-[13px]">

                                        <!-- Property card description -->
                                        <div class="text-gray-500">
                                            4936 N Broadway St, Chicago, IL 60640, USA
                                        </div>

                                    </div>

                                    <div class="flex justify-start items-center text-[15px] gap-x-3 mt-3">

                                        <!-- bed count -->
                                        <div class="flex justify-start items-center">

                                            <svg class="fill-black size-[15px]" viewBox="0 0 50 50" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path d="M5 10C3.355469 10 2 11.355469 2 13L2 28.1875C2.003906 28.25 2.015625 28.3125 2.03125 28.375C2.03125 28.386719 2.03125 28.394531 2.03125 28.40625C1.582031 29.113281 1.214844 29.867188 0.9375 30.6875C0.316406 32.519531 0.0507813 34.621094 0 37L0 38C0 38.03125 0 38.0625 0 38.09375L0 50L7 50L7 46C7 45.167969 7.203125 44.734375 7.46875 44.46875C7.734375 44.203125 8.167969 44 9 44L41 44C41.832031 44 42.265625 44.203125 42.53125 44.46875C42.796875 44.734375 43 45.167969 43 46L43 50L50 50L50 38.15625C50.003906 38.105469 50.003906 38.050781 50 38C50 37.65625 50.007813 37.332031 50 37C49.949219 34.621094 49.683594 32.519531 49.0625 30.6875C48.785156 29.875 48.414063 29.136719 47.96875 28.4375C47.988281 28.355469 48 28.273438 48 28.1875L48 13C48 11.355469 46.644531 10 45 10 Z M 5 12L45 12C45.5625 12 46 12.4375 46 13L46 26.15625C45.753906 25.949219 45.492188 25.75 45.21875 25.5625C44.550781 25.101563 43.824219 24.671875 43 24.3125L43 20C43 19.296875 42.539063 18.75 42.03125 18.40625C41.523438 18.0625 40.902344 17.824219 40.125 17.625C38.570313 17.226563 36.386719 17 33.5 17C30.613281 17 28.429688 17.226563 26.875 17.625C26.117188 17.820313 25.5 18.042969 25 18.375C24.5 18.042969 23.882813 17.820313 23.125 17.625C21.570313 17.226563 19.386719 17 16.5 17C13.613281 17 11.429688 17.226563 9.875 17.625C9.097656 17.824219 8.476563 18.0625 7.96875 18.40625C7.460938 18.75 7 19.296875 7 20L7 24.3125C6.175781 24.671875 5.449219 25.101563 4.78125 25.5625C4.507813 25.75 4.246094 25.949219 4 26.15625L4 13C4 12.4375 4.4375 12 5 12 Z M 16.5 19C19.28125 19 21.34375 19.234375 22.625 19.5625C23.265625 19.726563 23.707031 19.925781 23.90625 20.0625C23.988281 20.117188 23.992188 20.125 24 20.125L24 22C17.425781 22.042969 12.558594 22.535156 9 23.625L9 20.125C9.007813 20.125 9.011719 20.117188 9.09375 20.0625C9.292969 19.925781 9.734375 19.726563 10.375 19.5625C11.65625 19.234375 13.71875 19 16.5 19 Z M 33.5 19C36.28125 19 38.34375 19.234375 39.625 19.5625C40.265625 19.726563 40.707031 19.925781 40.90625 20.0625C40.988281 20.117188 40.992188 20.125 41 20.125L41 23.625C37.441406 22.535156 32.574219 22.042969 26 22L26 20.125C26.007813 20.125 26.011719 20.117188 26.09375 20.0625C26.292969 19.925781 26.734375 19.726563 27.375 19.5625C28.65625 19.234375 30.71875 19 33.5 19 Z M 24.8125 24C24.917969 24.015625 25.019531 24.015625 25.125 24C25.15625 24 25.1875 24 25.21875 24C35.226563 24.015625 41.007813 25.0625 44.09375 27.1875C45.648438 28.257813 46.589844 29.585938 47.1875 31.34375C47.707031 32.875 47.917969 34.761719 47.96875 37L2.03125 37C2.082031 34.761719 2.292969 32.875 2.8125 31.34375C3.410156 29.585938 4.351563 28.257813 5.90625 27.1875C8.992188 25.058594 14.785156 24.011719 24.8125 24 Z M 2 39L48 39L48 48L45 48L45 46C45 44.832031 44.703125 43.765625 43.96875 43.03125C43.234375 42.296875 42.167969 42 41 42L9 42C7.832031 42 6.765625 42.296875 6.03125 43.03125C5.296875 43.765625 5 44.832031 5 46L5 48L2 48Z"></path>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 2 </span>
                                        </div>

                                        <!-- bath count -->
                                        <div class="flex justify-start items-center">

                                            <svg viewBox="0 0 45 45" class="size-[15px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="stroke-black" d="M27.7812 11.002V9.002C27.7812 7.897 26.8863 7.002 25.7812 7.002H14.7812C13.6762 7.002 12.7812 7.897 12.7812 9.002V36.002" stroke-width="2"></path>
                                                    <path class="stroke-black" d="M33.7812 11.002H21.7812V15.002H33.7812V11.002Z" stroke-width="2" stroke-linejoin="round"></path>
                                                    <path class="stroke-black" d="M18.7812 15.002H36.7812" stroke-width="2" stroke-linejoin="round"></path>
                                                    <path class="fill-black" d="M27.7812 18.002C27.2292 18.002 26.7812 18.45 26.7812 19.002C26.7812 19.554 27.2292 20.002 27.7812 20.002C28.3333 20.002 28.7812 19.554 28.7812 19.002C28.7812 18.45 28.3333 18.002 27.7812 18.002ZM22.7812 18.002C22.2293 18.002 21.7812 18.45 21.7812 19.002C21.7812 19.554 22.2293 20.002 22.7812 20.002C23.3333 20.002 23.7812 19.554 23.7812 19.002C23.7812 18.45 23.3333 18.002 22.7812 18.002ZM32.7812 18.002C32.2292 18.002 31.7812 18.45 31.7812 19.002C31.7812 19.554 32.2292 20.002 32.7812 20.002C33.3333 20.002 33.7812 19.554 33.7812 19.002C33.7812 18.45 33.3333 18.002 32.7812 18.002ZM27.7812 22.002C27.2292 22.002 26.7812 22.45 26.7812 23.002C26.7812 23.554 27.2292 24.002 27.7812 24.002C28.3333 24.002 28.7812 23.554 28.7812 23.002C28.7812 22.45 28.3333 22.002 27.7812 22.002ZM32.7812 22.002C32.2292 22.002 31.7812 22.45 31.7812 23.002C31.7812 23.554 32.2292 24.002 32.7812 24.002C33.3333 24.002 33.7812 23.554 33.7812 23.002C33.7812 22.45 33.3333 22.002 32.7812 22.002ZM22.7812 22.002C22.2293 22.002 21.7812 22.45 21.7812 23.002C21.7812 23.554 22.2293 24.002 22.7812 24.002C23.3333 24.002 23.7812 23.554 23.7812 23.002C23.7812 22.45 23.3333 22.002 22.7812 22.002ZM27.7812 26.002C27.2292 26.002 26.7812 26.45 26.7812 27.002C26.7812 27.554 27.2292 28.002 27.7812 28.002C28.3333 28.002 28.7812 27.554 28.7812 27.002C28.7812 26.45 28.3333 26.002 27.7812 26.002ZM32.7812 26.002C32.2292 26.002 31.7812 26.45 31.7812 27.002C31.7812 27.554 32.2292 28.002 32.7812 28.002C33.3333 28.002 33.7812 27.554 33.7812 27.002C33.7812 26.45 33.3333 26.002 32.7812 26.002ZM22.7812 26.002C22.2293 26.002 21.7812 26.45 21.7812 27.002C21.7812 27.554 22.2293 28.002 22.7812 28.002C23.3333 28.002 23.7812 27.554 23.7812 27.002C23.7812 26.45 23.3333 26.002 22.7812 26.002ZM27.7812 30.002C27.2292 30.002 26.7812 30.45 26.7812 31.002C26.7812 31.554 27.2292 32.002 27.7812 32.002C28.3333 32.002 28.7812 31.554 28.7812 31.002C28.7812 30.45 28.3333 30.002 27.7812 30.002ZM32.7812 30.002C32.2292 30.002 31.7812 30.45 31.7812 31.002C31.7812 31.554 32.2292 32.002 32.7812 32.002C33.3333 32.002 33.7812 31.554 33.7812 31.002C33.7812 30.45 33.3333 30.002 32.7812 30.002ZM22.7812 30.002C22.2293 30.002 21.7812 30.45 21.7812 31.002C21.7812 31.554 22.2293 32.002 22.7812 32.002C23.3333 32.002 23.7812 31.554 23.7812 31.002C23.7812 30.45 23.3333 30.002 22.7812 30.002ZM27.7812 34.002C27.2292 34.002 26.7812 34.45 26.7812 35.002C26.7812 35.554 27.2292 36.002 27.7812 36.002C28.3333 36.002 28.7812 35.554 28.7812 35.002C28.7812 34.45 28.3333 34.002 27.7812 34.002ZM32.7812 34.002C32.2292 34.002 31.7812 34.45 31.7812 35.002C31.7812 35.554 32.2292 36.002 32.7812 36.002C33.3333 36.002 33.7812 35.554 33.7812 35.002C33.7812 34.45 33.3333 34.002 32.7812 34.002ZM22.7812 34.002C22.2293 34.002 21.7812 34.45 21.7812 35.002C21.7812 35.554 22.2293 36.002 22.7812 36.002C23.3333 36.002 23.7812 35.554 23.7812 35.002C23.7812 34.45 23.3333 34.002 22.7812 34.002Z" fill="#3C3C3C"></path> <path d="M32.7812 18.002C32.2292 18.002 31.7812 18.45 31.7812 19.002C31.7812 19.554 32.2292 20.002 32.7812 20.002C33.3333 20.002 33.7812 19.554 33.7812 19.002C33.7812 18.45 33.3333 18.002 32.7812 18.002ZM27.7812 18.002C27.2293 18.002 26.7812 18.45 26.7812 19.002C26.7812 19.554 27.2293 20.002 27.7812 20.002C28.3333 20.002 28.7812 19.554 28.7812 19.002C28.7812 18.45 28.3333 18.002 27.7812 18.002ZM32.7812 22.002C32.2292 22.002 31.7812 22.45 31.7812 23.002C31.7812 23.554 32.2292 24.002 32.7812 24.002C33.3333 24.002 33.7812 23.554 33.7812 23.002C33.7812 22.45 33.3333 22.002 32.7812 22.002ZM27.7812 22.002C27.2293 22.002 26.7812 22.45 26.7812 23.002C26.7812 23.554 27.2293 24.002 27.7812 24.002C28.3333 24.002 28.7812 23.554 28.7812 23.002C28.7812 22.45 28.3333 22.002 27.7812 22.002ZM32.7812 26.002C32.2292 26.002 31.7812 26.45 31.7812 27.002C31.7812 27.554 32.2292 28.002 32.7812 28.002C33.3333 28.002 33.7812 27.554 33.7812 27.002C33.7812 26.45 33.3333 26.002 32.7812 26.002ZM27.7812 26.002C27.2293 26.002 26.7812 26.45 26.7812 27.002C26.7812 27.554 27.2293 28.002 27.7812 28.002C28.3333 28.002 28.7812 27.554 28.7812 27.002C28.7812 26.45 28.3333 26.002 27.7812 26.002ZM32.7812 30.002C32.2292 30.002 31.7812 30.45 31.7812 31.002C31.7812 31.554 32.2292 32.002 32.7812 32.002C33.3333 32.002 33.7812 31.554 33.7812 31.002C33.7812 30.45 33.3333 30.002 32.7812 30.002ZM27.7812 30.002C27.2293 30.002 26.7812 30.45 26.7812 31.002C26.7812 31.554 27.2293 32.002 27.7812 32.002C28.3333 32.002 28.7812 31.554 28.7812 31.002C28.7812 30.45 28.3333 30.002 27.7812 30.002ZM32.7812 34.002C32.2292 34.002 31.7812 34.45 31.7812 35.002C31.7812 35.554 32.2292 36.002 32.7812 36.002C33.3333 36.002 33.7812 35.554 33.7812 35.002C33.7812 34.45 33.3333 34.002 32.7812 34.002ZM27.7812 34.002C27.2293 34.002 26.7812 34.45 26.7812 35.002C26.7812 35.554 27.2293 36.002 27.7812 36.002C28.3333 36.002 28.7812 35.554 28.7812 35.002C28.7812 34.45 28.3333 34.002 27.7812 34.002Z"></path>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 2 </span>
                                        </div>

                                        <!-- sq ft count -->
                                        <div class="flex justify-start items-center">

                                            <svg class="fill-black size-[15px]" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 297 297" xml:space="preserve">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <g>
                                                        <path d="M285.491,1.703c-4.467-1.284-9.238,0.551-11.699,4.49L101.994,281.069c-2.006,3.209-2.112,7.255-0.276,10.565 c1.834,3.311,5.321,5.365,9.105,5.365h171.798c5.75,0,10.412-4.662,10.412-10.412V11.712 C293.033,7.067,289.956,2.983,285.491,1.703z M272.209,276.176h-13.535v-6.768c0-5.75-4.662-10.412-10.412-10.412 s-10.412,4.662-10.412,10.412v6.768h-24.988v-6.768c0-5.75-4.662-10.412-10.412-10.412s-10.412,4.662-10.412,10.412v6.768h-24.989 v-6.768c0-5.75-4.661-10.412-10.411-10.412s-10.412,4.662-10.412,10.412v6.768H129.61L272.209,48.017V276.176z"></path>
                                                        <path d="M193.635,243.53h40.535c5.75,0,10.412-4.661,10.412-10.411V168.26c0-4.645-3.077-8.729-7.542-10.009 c-4.469-1.284-9.237,0.552-11.7,4.49l-40.535,64.859c-2.005,3.21-2.112,7.255-0.276,10.565 C186.362,241.477,189.849,243.53,193.635,243.53z M223.758,204.565v18.142H212.42L223.758,204.565z"></path>
                                                        <path d="M217.984,44.632c3.024-4.89,1.517-11.303-3.37-14.331l-46.387-28.74c-2.349-1.453-5.18-1.916-7.866-1.285 c-2.689,0.633-5.018,2.308-6.472,4.656l-4.371,7.063c-20.907-10.192-44.484-12.863-67.43-7.462 c-26.352,6.201-48.715,22.299-62.967,45.328C-8.446,94.412,2.747,152.213,43.364,183.531l-4.377,7.074 c-3.024,4.888-1.517,11.302,3.369,14.33l46.386,28.745c1.66,1.029,3.562,1.563,5.484,1.563c0.796,0,1.596-0.092,2.384-0.277 c2.689-0.632,5.017-2.307,6.471-4.656L217.984,44.632z M36.83,60.82c11.324-18.299,29.092-31.09,50.03-36.018 c17.521-4.122,35.5-2.303,51.618,5.033l-16.061,25.953c-9.116-3.315-19.003-3.928-28.668-1.651 C80.637,57.222,69.512,65.231,62.418,76.69c-12.755,20.615-8.822,46.932,8.05,63.046L54.41,165.684 C23.526,140.51,15.311,95.594,36.83,60.82z M81.853,121.338c-7.299-9.497-8.393-22.919-1.727-33.689 c4.163-6.728,10.695-11.431,18.393-13.241c4.179-0.984,8.43-1.043,12.52-0.23L81.853,121.338z M90.854,210.492l-28.684-17.775 L166.116,24.749l28.685,17.773l-8.193,13.241l-6.606-4.094c-4.889-3.03-11.306-1.522-14.334,3.365 c-3.029,4.888-1.523,11.306,3.365,14.335l6.617,4.101l-14.576,23.553l-6.607-4.093c-4.89-3.027-11.307-1.521-14.335,3.367 c-3.028,4.888-1.522,11.306,3.367,14.335l6.616,4.1l-14.574,23.553l-6.607-4.094c-4.889-3.026-11.307-1.52-14.335,3.368 c-3.027,4.889-1.521,11.306,3.368,14.334l6.616,4.1l-14.576,23.552l-6.604-4.094c-4.889-3.03-11.307-1.523-14.336,3.364 c-3.029,4.888-1.523,11.306,3.364,14.336l6.617,4.102L90.854,210.492z"></path>
                                                    </g>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 1560 Sq Ft </span>

                                        </div>

                                    </div>

                                </div>
                                <div>

                                    <!-- Property card on sale -->
                                    <div class="mb-3">
                                        <div class="mb-1 text-[13px] text-blue-900"> On sale </div>
                                        <div class="text-[18px]"> $540,000 </div>
                                    </div>

                                    <!-- Property card on author -->
                                    <div>
                                        <div class="text-[13px] text-blue-900"> By </div>
                                        <div class="text-[18px]"> Maria Villalobos </div>
                                    </div>

                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Property card -->
                    <div class="decoration-0 w-full">
                        <div class="mb-5 border flex flex-wrap duration-500 hover:shadow-xl group">
                            <div class="w-full sm:w-1/2">

                                <div class="relative">

                                    <!-- Property card image -->
                                    <img :src="'/images/properties-3.jpg'" class="object-cover bg-cover w-full h-[250px]" alt="properties"/>

                                    <div class="absolute bottom-0 start-0 top-0 end-0 p-3 flex justify-center items-center gap-x-3 bg-blue-200 bg-opacity-45 scale-0 duration-500 group-hover:scale-100">
                                        <button type="button" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill="#ffffff" d="M16.5562 12.9062L16.1007 13.359C16.1007 13.359 15.0181 14.4355 12.0631 11.4972C9.10812 8.55901 10.1907 7.48257 10.1907 7.48257L10.4775 7.19738C11.1841 6.49484 11.2507 5.36691 10.6342 4.54348L9.37326 2.85908C8.61028 1.83992 7.13596 1.70529 6.26145 2.57483L4.69185 4.13552C4.25823 4.56668 3.96765 5.12559 4.00289 5.74561C4.09304 7.33182 4.81071 10.7447 8.81536 14.7266C13.0621 18.9492 17.0468 19.117 18.6763 18.9651C19.1917 18.9171 19.6399 18.6546 20.0011 18.2954L21.4217 16.883C22.3806 15.9295 22.1102 14.2949 20.8833 13.628L18.9728 12.5894C18.1672 12.1515 17.1858 12.2801 16.5562 12.9062Z"></path>
                                                </g>
                                            </svg>
                                        </button>
                                        <button type="button" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="stroke-white duration-500 group-hover/item:stroke-blue-600" d="M4 7.00005L10.2 11.65C11.2667 12.45 12.7333 12.45 13.8 11.65L20 7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                    <rect class="stroke-white duration-500 group-hover/item:stroke-blue-600" x="3" y="5" width="18" height="14" rx="2" stroke-width="2" stroke-linecap="round"></rect>
                                                </g>
                                            </svg>
                                        </button>
                                        <router-link :to="{name: 'PropertyDetails'}" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M11.9944 15.5C13.9274 15.5 15.4944 13.933 15.4944 12C15.4944 10.067 13.9274 8.5 11.9944 8.5C10.0614 8.5 8.49439 10.067 8.49439 12C8.49439 13.933 10.0614 15.5 11.9944 15.5ZM11.9944 13.4944C11.1691 13.4944 10.5 12.8253 10.5 12C10.5 11.1747 11.1691 10.5056 11.9944 10.5056C12.8197 10.5056 13.4888 11.1747 13.4888 12C13.4888 12.8253 12.8197 13.4944 11.9944 13.4944Z"></path>
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M12 5C7.18879 5 3.9167 7.60905 2.1893 9.47978C0.857392 10.9222 0.857393 13.0778 2.1893 14.5202C3.9167 16.391 7.18879 19 12 19C16.8112 19 20.0833 16.391 21.8107 14.5202C23.1426 13.0778 23.1426 10.9222 21.8107 9.47978C20.0833 7.60905 16.8112 5 12 5ZM3.65868 10.8366C5.18832 9.18002 7.9669 7 12 7C16.0331 7 18.8117 9.18002 20.3413 10.8366C20.9657 11.5128 20.9657 12.4872 20.3413 13.1634C18.8117 14.82 16.0331 17 12 17C7.9669 17 5.18832 14.82 3.65868 13.1634C3.03426 12.4872 3.03426 11.5128 3.65868 10.8366Z"></path>
                                                </g>
                                            </svg>
                                        </router-link>
                                    </div>

                                </div>

                            </div>
                            <div class="w-full sm:w-1/2 p-3 py-3 flex justify-between flex-col">
                                <div class="mb-3">
                                    <div class="mb-3 text-[21px]">

                                        <!-- Property card title -->
                                        <div class="truncate-to-1-line">
                                            Modern Apartment
                                        </div>
                                    </div>
                                    <div class="text-[13px]">

                                        <!-- Property card description -->
                                        <div class="text-gray-500">
                                            4936 N Broadway St, Chicago, IL 60640, USA
                                        </div>

                                    </div>

                                    <div class="flex justify-start items-center text-[15px] gap-x-3 mt-3">

                                        <!-- bed count -->
                                        <div class="flex justify-start items-center">

                                            <svg class="fill-black size-[15px]" viewBox="0 0 50 50" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path d="M5 10C3.355469 10 2 11.355469 2 13L2 28.1875C2.003906 28.25 2.015625 28.3125 2.03125 28.375C2.03125 28.386719 2.03125 28.394531 2.03125 28.40625C1.582031 29.113281 1.214844 29.867188 0.9375 30.6875C0.316406 32.519531 0.0507813 34.621094 0 37L0 38C0 38.03125 0 38.0625 0 38.09375L0 50L7 50L7 46C7 45.167969 7.203125 44.734375 7.46875 44.46875C7.734375 44.203125 8.167969 44 9 44L41 44C41.832031 44 42.265625 44.203125 42.53125 44.46875C42.796875 44.734375 43 45.167969 43 46L43 50L50 50L50 38.15625C50.003906 38.105469 50.003906 38.050781 50 38C50 37.65625 50.007813 37.332031 50 37C49.949219 34.621094 49.683594 32.519531 49.0625 30.6875C48.785156 29.875 48.414063 29.136719 47.96875 28.4375C47.988281 28.355469 48 28.273438 48 28.1875L48 13C48 11.355469 46.644531 10 45 10 Z M 5 12L45 12C45.5625 12 46 12.4375 46 13L46 26.15625C45.753906 25.949219 45.492188 25.75 45.21875 25.5625C44.550781 25.101563 43.824219 24.671875 43 24.3125L43 20C43 19.296875 42.539063 18.75 42.03125 18.40625C41.523438 18.0625 40.902344 17.824219 40.125 17.625C38.570313 17.226563 36.386719 17 33.5 17C30.613281 17 28.429688 17.226563 26.875 17.625C26.117188 17.820313 25.5 18.042969 25 18.375C24.5 18.042969 23.882813 17.820313 23.125 17.625C21.570313 17.226563 19.386719 17 16.5 17C13.613281 17 11.429688 17.226563 9.875 17.625C9.097656 17.824219 8.476563 18.0625 7.96875 18.40625C7.460938 18.75 7 19.296875 7 20L7 24.3125C6.175781 24.671875 5.449219 25.101563 4.78125 25.5625C4.507813 25.75 4.246094 25.949219 4 26.15625L4 13C4 12.4375 4.4375 12 5 12 Z M 16.5 19C19.28125 19 21.34375 19.234375 22.625 19.5625C23.265625 19.726563 23.707031 19.925781 23.90625 20.0625C23.988281 20.117188 23.992188 20.125 24 20.125L24 22C17.425781 22.042969 12.558594 22.535156 9 23.625L9 20.125C9.007813 20.125 9.011719 20.117188 9.09375 20.0625C9.292969 19.925781 9.734375 19.726563 10.375 19.5625C11.65625 19.234375 13.71875 19 16.5 19 Z M 33.5 19C36.28125 19 38.34375 19.234375 39.625 19.5625C40.265625 19.726563 40.707031 19.925781 40.90625 20.0625C40.988281 20.117188 40.992188 20.125 41 20.125L41 23.625C37.441406 22.535156 32.574219 22.042969 26 22L26 20.125C26.007813 20.125 26.011719 20.117188 26.09375 20.0625C26.292969 19.925781 26.734375 19.726563 27.375 19.5625C28.65625 19.234375 30.71875 19 33.5 19 Z M 24.8125 24C24.917969 24.015625 25.019531 24.015625 25.125 24C25.15625 24 25.1875 24 25.21875 24C35.226563 24.015625 41.007813 25.0625 44.09375 27.1875C45.648438 28.257813 46.589844 29.585938 47.1875 31.34375C47.707031 32.875 47.917969 34.761719 47.96875 37L2.03125 37C2.082031 34.761719 2.292969 32.875 2.8125 31.34375C3.410156 29.585938 4.351563 28.257813 5.90625 27.1875C8.992188 25.058594 14.785156 24.011719 24.8125 24 Z M 2 39L48 39L48 48L45 48L45 46C45 44.832031 44.703125 43.765625 43.96875 43.03125C43.234375 42.296875 42.167969 42 41 42L9 42C7.832031 42 6.765625 42.296875 6.03125 43.03125C5.296875 43.765625 5 44.832031 5 46L5 48L2 48Z"></path>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 2 </span>
                                        </div>

                                        <!-- bath count -->
                                        <div class="flex justify-start items-center">

                                            <svg viewBox="0 0 45 45" class="size-[15px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="stroke-black" d="M27.7812 11.002V9.002C27.7812 7.897 26.8863 7.002 25.7812 7.002H14.7812C13.6762 7.002 12.7812 7.897 12.7812 9.002V36.002" stroke-width="2"></path>
                                                    <path class="stroke-black" d="M33.7812 11.002H21.7812V15.002H33.7812V11.002Z" stroke-width="2" stroke-linejoin="round"></path>
                                                    <path class="stroke-black" d="M18.7812 15.002H36.7812" stroke-width="2" stroke-linejoin="round"></path>
                                                    <path class="fill-black" d="M27.7812 18.002C27.2292 18.002 26.7812 18.45 26.7812 19.002C26.7812 19.554 27.2292 20.002 27.7812 20.002C28.3333 20.002 28.7812 19.554 28.7812 19.002C28.7812 18.45 28.3333 18.002 27.7812 18.002ZM22.7812 18.002C22.2293 18.002 21.7812 18.45 21.7812 19.002C21.7812 19.554 22.2293 20.002 22.7812 20.002C23.3333 20.002 23.7812 19.554 23.7812 19.002C23.7812 18.45 23.3333 18.002 22.7812 18.002ZM32.7812 18.002C32.2292 18.002 31.7812 18.45 31.7812 19.002C31.7812 19.554 32.2292 20.002 32.7812 20.002C33.3333 20.002 33.7812 19.554 33.7812 19.002C33.7812 18.45 33.3333 18.002 32.7812 18.002ZM27.7812 22.002C27.2292 22.002 26.7812 22.45 26.7812 23.002C26.7812 23.554 27.2292 24.002 27.7812 24.002C28.3333 24.002 28.7812 23.554 28.7812 23.002C28.7812 22.45 28.3333 22.002 27.7812 22.002ZM32.7812 22.002C32.2292 22.002 31.7812 22.45 31.7812 23.002C31.7812 23.554 32.2292 24.002 32.7812 24.002C33.3333 24.002 33.7812 23.554 33.7812 23.002C33.7812 22.45 33.3333 22.002 32.7812 22.002ZM22.7812 22.002C22.2293 22.002 21.7812 22.45 21.7812 23.002C21.7812 23.554 22.2293 24.002 22.7812 24.002C23.3333 24.002 23.7812 23.554 23.7812 23.002C23.7812 22.45 23.3333 22.002 22.7812 22.002ZM27.7812 26.002C27.2292 26.002 26.7812 26.45 26.7812 27.002C26.7812 27.554 27.2292 28.002 27.7812 28.002C28.3333 28.002 28.7812 27.554 28.7812 27.002C28.7812 26.45 28.3333 26.002 27.7812 26.002ZM32.7812 26.002C32.2292 26.002 31.7812 26.45 31.7812 27.002C31.7812 27.554 32.2292 28.002 32.7812 28.002C33.3333 28.002 33.7812 27.554 33.7812 27.002C33.7812 26.45 33.3333 26.002 32.7812 26.002ZM22.7812 26.002C22.2293 26.002 21.7812 26.45 21.7812 27.002C21.7812 27.554 22.2293 28.002 22.7812 28.002C23.3333 28.002 23.7812 27.554 23.7812 27.002C23.7812 26.45 23.3333 26.002 22.7812 26.002ZM27.7812 30.002C27.2292 30.002 26.7812 30.45 26.7812 31.002C26.7812 31.554 27.2292 32.002 27.7812 32.002C28.3333 32.002 28.7812 31.554 28.7812 31.002C28.7812 30.45 28.3333 30.002 27.7812 30.002ZM32.7812 30.002C32.2292 30.002 31.7812 30.45 31.7812 31.002C31.7812 31.554 32.2292 32.002 32.7812 32.002C33.3333 32.002 33.7812 31.554 33.7812 31.002C33.7812 30.45 33.3333 30.002 32.7812 30.002ZM22.7812 30.002C22.2293 30.002 21.7812 30.45 21.7812 31.002C21.7812 31.554 22.2293 32.002 22.7812 32.002C23.3333 32.002 23.7812 31.554 23.7812 31.002C23.7812 30.45 23.3333 30.002 22.7812 30.002ZM27.7812 34.002C27.2292 34.002 26.7812 34.45 26.7812 35.002C26.7812 35.554 27.2292 36.002 27.7812 36.002C28.3333 36.002 28.7812 35.554 28.7812 35.002C28.7812 34.45 28.3333 34.002 27.7812 34.002ZM32.7812 34.002C32.2292 34.002 31.7812 34.45 31.7812 35.002C31.7812 35.554 32.2292 36.002 32.7812 36.002C33.3333 36.002 33.7812 35.554 33.7812 35.002C33.7812 34.45 33.3333 34.002 32.7812 34.002ZM22.7812 34.002C22.2293 34.002 21.7812 34.45 21.7812 35.002C21.7812 35.554 22.2293 36.002 22.7812 36.002C23.3333 36.002 23.7812 35.554 23.7812 35.002C23.7812 34.45 23.3333 34.002 22.7812 34.002Z" fill="#3C3C3C"></path> <path d="M32.7812 18.002C32.2292 18.002 31.7812 18.45 31.7812 19.002C31.7812 19.554 32.2292 20.002 32.7812 20.002C33.3333 20.002 33.7812 19.554 33.7812 19.002C33.7812 18.45 33.3333 18.002 32.7812 18.002ZM27.7812 18.002C27.2293 18.002 26.7812 18.45 26.7812 19.002C26.7812 19.554 27.2293 20.002 27.7812 20.002C28.3333 20.002 28.7812 19.554 28.7812 19.002C28.7812 18.45 28.3333 18.002 27.7812 18.002ZM32.7812 22.002C32.2292 22.002 31.7812 22.45 31.7812 23.002C31.7812 23.554 32.2292 24.002 32.7812 24.002C33.3333 24.002 33.7812 23.554 33.7812 23.002C33.7812 22.45 33.3333 22.002 32.7812 22.002ZM27.7812 22.002C27.2293 22.002 26.7812 22.45 26.7812 23.002C26.7812 23.554 27.2293 24.002 27.7812 24.002C28.3333 24.002 28.7812 23.554 28.7812 23.002C28.7812 22.45 28.3333 22.002 27.7812 22.002ZM32.7812 26.002C32.2292 26.002 31.7812 26.45 31.7812 27.002C31.7812 27.554 32.2292 28.002 32.7812 28.002C33.3333 28.002 33.7812 27.554 33.7812 27.002C33.7812 26.45 33.3333 26.002 32.7812 26.002ZM27.7812 26.002C27.2293 26.002 26.7812 26.45 26.7812 27.002C26.7812 27.554 27.2293 28.002 27.7812 28.002C28.3333 28.002 28.7812 27.554 28.7812 27.002C28.7812 26.45 28.3333 26.002 27.7812 26.002ZM32.7812 30.002C32.2292 30.002 31.7812 30.45 31.7812 31.002C31.7812 31.554 32.2292 32.002 32.7812 32.002C33.3333 32.002 33.7812 31.554 33.7812 31.002C33.7812 30.45 33.3333 30.002 32.7812 30.002ZM27.7812 30.002C27.2293 30.002 26.7812 30.45 26.7812 31.002C26.7812 31.554 27.2293 32.002 27.7812 32.002C28.3333 32.002 28.7812 31.554 28.7812 31.002C28.7812 30.45 28.3333 30.002 27.7812 30.002ZM32.7812 34.002C32.2292 34.002 31.7812 34.45 31.7812 35.002C31.7812 35.554 32.2292 36.002 32.7812 36.002C33.3333 36.002 33.7812 35.554 33.7812 35.002C33.7812 34.45 33.3333 34.002 32.7812 34.002ZM27.7812 34.002C27.2293 34.002 26.7812 34.45 26.7812 35.002C26.7812 35.554 27.2293 36.002 27.7812 36.002C28.3333 36.002 28.7812 35.554 28.7812 35.002C28.7812 34.45 28.3333 34.002 27.7812 34.002Z"></path>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 2 </span>
                                        </div>

                                        <!-- sq ft count -->
                                        <div class="flex justify-start items-center">

                                            <svg class="fill-black size-[15px]" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 297 297" xml:space="preserve">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <g>
                                                        <path d="M285.491,1.703c-4.467-1.284-9.238,0.551-11.699,4.49L101.994,281.069c-2.006,3.209-2.112,7.255-0.276,10.565 c1.834,3.311,5.321,5.365,9.105,5.365h171.798c5.75,0,10.412-4.662,10.412-10.412V11.712 C293.033,7.067,289.956,2.983,285.491,1.703z M272.209,276.176h-13.535v-6.768c0-5.75-4.662-10.412-10.412-10.412 s-10.412,4.662-10.412,10.412v6.768h-24.988v-6.768c0-5.75-4.662-10.412-10.412-10.412s-10.412,4.662-10.412,10.412v6.768h-24.989 v-6.768c0-5.75-4.661-10.412-10.411-10.412s-10.412,4.662-10.412,10.412v6.768H129.61L272.209,48.017V276.176z"></path>
                                                        <path d="M193.635,243.53h40.535c5.75,0,10.412-4.661,10.412-10.411V168.26c0-4.645-3.077-8.729-7.542-10.009 c-4.469-1.284-9.237,0.552-11.7,4.49l-40.535,64.859c-2.005,3.21-2.112,7.255-0.276,10.565 C186.362,241.477,189.849,243.53,193.635,243.53z M223.758,204.565v18.142H212.42L223.758,204.565z"></path>
                                                        <path d="M217.984,44.632c3.024-4.89,1.517-11.303-3.37-14.331l-46.387-28.74c-2.349-1.453-5.18-1.916-7.866-1.285 c-2.689,0.633-5.018,2.308-6.472,4.656l-4.371,7.063c-20.907-10.192-44.484-12.863-67.43-7.462 c-26.352,6.201-48.715,22.299-62.967,45.328C-8.446,94.412,2.747,152.213,43.364,183.531l-4.377,7.074 c-3.024,4.888-1.517,11.302,3.369,14.33l46.386,28.745c1.66,1.029,3.562,1.563,5.484,1.563c0.796,0,1.596-0.092,2.384-0.277 c2.689-0.632,5.017-2.307,6.471-4.656L217.984,44.632z M36.83,60.82c11.324-18.299,29.092-31.09,50.03-36.018 c17.521-4.122,35.5-2.303,51.618,5.033l-16.061,25.953c-9.116-3.315-19.003-3.928-28.668-1.651 C80.637,57.222,69.512,65.231,62.418,76.69c-12.755,20.615-8.822,46.932,8.05,63.046L54.41,165.684 C23.526,140.51,15.311,95.594,36.83,60.82z M81.853,121.338c-7.299-9.497-8.393-22.919-1.727-33.689 c4.163-6.728,10.695-11.431,18.393-13.241c4.179-0.984,8.43-1.043,12.52-0.23L81.853,121.338z M90.854,210.492l-28.684-17.775 L166.116,24.749l28.685,17.773l-8.193,13.241l-6.606-4.094c-4.889-3.03-11.306-1.522-14.334,3.365 c-3.029,4.888-1.523,11.306,3.365,14.335l6.617,4.101l-14.576,23.553l-6.607-4.093c-4.89-3.027-11.307-1.521-14.335,3.367 c-3.028,4.888-1.522,11.306,3.367,14.335l6.616,4.1l-14.574,23.553l-6.607-4.094c-4.889-3.026-11.307-1.52-14.335,3.368 c-3.027,4.889-1.521,11.306,3.368,14.334l6.616,4.1l-14.576,23.552l-6.604-4.094c-4.889-3.03-11.307-1.523-14.336,3.364 c-3.029,4.888-1.523,11.306,3.364,14.336l6.617,4.102L90.854,210.492z"></path>
                                                    </g>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 1560 Sq Ft </span>

                                        </div>

                                    </div>

                                </div>
                                <div>

                                    <!-- Property card on sale -->
                                    <div class="mb-3">
                                        <div class="mb-1 text-[13px] text-blue-900"> On sale </div>
                                        <div class="text-[18px]"> $540,000 </div>
                                    </div>

                                    <!-- Property card on author -->
                                    <div>
                                        <div class="text-[13px] text-blue-900"> By </div>
                                        <div class="text-[18px]"> Maria Villalobos </div>
                                    </div>

                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Property card -->
                    <div class="decoration-0 w-full">
                        <div class="mb-5 border flex flex-wrap duration-500 hover:shadow-xl group">
                            <div class="w-full sm:w-1/2">

                                <div class="relative">

                                    <!-- Property card image -->
                                    <img :src="'/images/properties-4.jpg'" class="object-cover bg-cover w-full h-[250px]" alt="properties"/>

                                    <div class="absolute bottom-0 start-0 top-0 end-0 p-3 flex justify-center items-center gap-x-3 bg-blue-200 bg-opacity-45 scale-0 duration-500 group-hover:scale-100">
                                        <button type="button" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill="#ffffff" d="M16.5562 12.9062L16.1007 13.359C16.1007 13.359 15.0181 14.4355 12.0631 11.4972C9.10812 8.55901 10.1907 7.48257 10.1907 7.48257L10.4775 7.19738C11.1841 6.49484 11.2507 5.36691 10.6342 4.54348L9.37326 2.85908C8.61028 1.83992 7.13596 1.70529 6.26145 2.57483L4.69185 4.13552C4.25823 4.56668 3.96765 5.12559 4.00289 5.74561C4.09304 7.33182 4.81071 10.7447 8.81536 14.7266C13.0621 18.9492 17.0468 19.117 18.6763 18.9651C19.1917 18.9171 19.6399 18.6546 20.0011 18.2954L21.4217 16.883C22.3806 15.9295 22.1102 14.2949 20.8833 13.628L18.9728 12.5894C18.1672 12.1515 17.1858 12.2801 16.5562 12.9062Z"></path>
                                                </g>
                                            </svg>
                                        </button>
                                        <button type="button" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="stroke-white duration-500 group-hover/item:stroke-blue-600" d="M4 7.00005L10.2 11.65C11.2667 12.45 12.7333 12.45 13.8 11.65L20 7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                    <rect class="stroke-white duration-500 group-hover/item:stroke-blue-600" x="3" y="5" width="18" height="14" rx="2" stroke-width="2" stroke-linecap="round"></rect>
                                                </g>
                                            </svg>
                                        </button>
                                        <router-link :to="{name: 'PropertyDetails'}" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M11.9944 15.5C13.9274 15.5 15.4944 13.933 15.4944 12C15.4944 10.067 13.9274 8.5 11.9944 8.5C10.0614 8.5 8.49439 10.067 8.49439 12C8.49439 13.933 10.0614 15.5 11.9944 15.5ZM11.9944 13.4944C11.1691 13.4944 10.5 12.8253 10.5 12C10.5 11.1747 11.1691 10.5056 11.9944 10.5056C12.8197 10.5056 13.4888 11.1747 13.4888 12C13.4888 12.8253 12.8197 13.4944 11.9944 13.4944Z"></path>
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M12 5C7.18879 5 3.9167 7.60905 2.1893 9.47978C0.857392 10.9222 0.857393 13.0778 2.1893 14.5202C3.9167 16.391 7.18879 19 12 19C16.8112 19 20.0833 16.391 21.8107 14.5202C23.1426 13.0778 23.1426 10.9222 21.8107 9.47978C20.0833 7.60905 16.8112 5 12 5ZM3.65868 10.8366C5.18832 9.18002 7.9669 7 12 7C16.0331 7 18.8117 9.18002 20.3413 10.8366C20.9657 11.5128 20.9657 12.4872 20.3413 13.1634C18.8117 14.82 16.0331 17 12 17C7.9669 17 5.18832 14.82 3.65868 13.1634C3.03426 12.4872 3.03426 11.5128 3.65868 10.8366Z"></path>
                                                </g>
                                            </svg>
                                        </router-link>
                                    </div>

                                </div>

                            </div>
                            <div class="w-full sm:w-1/2 p-3 py-3 flex justify-between flex-col">
                                <div class="mb-3">
                                    <div class="mb-3 text-[21px]">

                                        <!-- Property card title -->
                                        <div class="truncate-to-1-line">
                                            Modern Apartment
                                        </div>
                                    </div>
                                    <div class="text-[13px]">

                                        <!-- Property card description -->
                                        <div class="text-gray-500">
                                            4936 N Broadway St, Chicago, IL 60640, USA
                                        </div>

                                    </div>

                                    <div class="flex justify-start items-center text-[15px] gap-x-3 mt-3">

                                        <!-- bed count -->
                                        <div class="flex justify-start items-center">

                                            <svg class="fill-black size-[15px]" viewBox="0 0 50 50" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path d="M5 10C3.355469 10 2 11.355469 2 13L2 28.1875C2.003906 28.25 2.015625 28.3125 2.03125 28.375C2.03125 28.386719 2.03125 28.394531 2.03125 28.40625C1.582031 29.113281 1.214844 29.867188 0.9375 30.6875C0.316406 32.519531 0.0507813 34.621094 0 37L0 38C0 38.03125 0 38.0625 0 38.09375L0 50L7 50L7 46C7 45.167969 7.203125 44.734375 7.46875 44.46875C7.734375 44.203125 8.167969 44 9 44L41 44C41.832031 44 42.265625 44.203125 42.53125 44.46875C42.796875 44.734375 43 45.167969 43 46L43 50L50 50L50 38.15625C50.003906 38.105469 50.003906 38.050781 50 38C50 37.65625 50.007813 37.332031 50 37C49.949219 34.621094 49.683594 32.519531 49.0625 30.6875C48.785156 29.875 48.414063 29.136719 47.96875 28.4375C47.988281 28.355469 48 28.273438 48 28.1875L48 13C48 11.355469 46.644531 10 45 10 Z M 5 12L45 12C45.5625 12 46 12.4375 46 13L46 26.15625C45.753906 25.949219 45.492188 25.75 45.21875 25.5625C44.550781 25.101563 43.824219 24.671875 43 24.3125L43 20C43 19.296875 42.539063 18.75 42.03125 18.40625C41.523438 18.0625 40.902344 17.824219 40.125 17.625C38.570313 17.226563 36.386719 17 33.5 17C30.613281 17 28.429688 17.226563 26.875 17.625C26.117188 17.820313 25.5 18.042969 25 18.375C24.5 18.042969 23.882813 17.820313 23.125 17.625C21.570313 17.226563 19.386719 17 16.5 17C13.613281 17 11.429688 17.226563 9.875 17.625C9.097656 17.824219 8.476563 18.0625 7.96875 18.40625C7.460938 18.75 7 19.296875 7 20L7 24.3125C6.175781 24.671875 5.449219 25.101563 4.78125 25.5625C4.507813 25.75 4.246094 25.949219 4 26.15625L4 13C4 12.4375 4.4375 12 5 12 Z M 16.5 19C19.28125 19 21.34375 19.234375 22.625 19.5625C23.265625 19.726563 23.707031 19.925781 23.90625 20.0625C23.988281 20.117188 23.992188 20.125 24 20.125L24 22C17.425781 22.042969 12.558594 22.535156 9 23.625L9 20.125C9.007813 20.125 9.011719 20.117188 9.09375 20.0625C9.292969 19.925781 9.734375 19.726563 10.375 19.5625C11.65625 19.234375 13.71875 19 16.5 19 Z M 33.5 19C36.28125 19 38.34375 19.234375 39.625 19.5625C40.265625 19.726563 40.707031 19.925781 40.90625 20.0625C40.988281 20.117188 40.992188 20.125 41 20.125L41 23.625C37.441406 22.535156 32.574219 22.042969 26 22L26 20.125C26.007813 20.125 26.011719 20.117188 26.09375 20.0625C26.292969 19.925781 26.734375 19.726563 27.375 19.5625C28.65625 19.234375 30.71875 19 33.5 19 Z M 24.8125 24C24.917969 24.015625 25.019531 24.015625 25.125 24C25.15625 24 25.1875 24 25.21875 24C35.226563 24.015625 41.007813 25.0625 44.09375 27.1875C45.648438 28.257813 46.589844 29.585938 47.1875 31.34375C47.707031 32.875 47.917969 34.761719 47.96875 37L2.03125 37C2.082031 34.761719 2.292969 32.875 2.8125 31.34375C3.410156 29.585938 4.351563 28.257813 5.90625 27.1875C8.992188 25.058594 14.785156 24.011719 24.8125 24 Z M 2 39L48 39L48 48L45 48L45 46C45 44.832031 44.703125 43.765625 43.96875 43.03125C43.234375 42.296875 42.167969 42 41 42L9 42C7.832031 42 6.765625 42.296875 6.03125 43.03125C5.296875 43.765625 5 44.832031 5 46L5 48L2 48Z"></path>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 2 </span>
                                        </div>

                                        <!-- bath count -->
                                        <div class="flex justify-start items-center">

                                            <svg viewBox="0 0 45 45" class="size-[15px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="stroke-black" d="M27.7812 11.002V9.002C27.7812 7.897 26.8863 7.002 25.7812 7.002H14.7812C13.6762 7.002 12.7812 7.897 12.7812 9.002V36.002" stroke-width="2"></path>
                                                    <path class="stroke-black" d="M33.7812 11.002H21.7812V15.002H33.7812V11.002Z" stroke-width="2" stroke-linejoin="round"></path>
                                                    <path class="stroke-black" d="M18.7812 15.002H36.7812" stroke-width="2" stroke-linejoin="round"></path>
                                                    <path class="fill-black" d="M27.7812 18.002C27.2292 18.002 26.7812 18.45 26.7812 19.002C26.7812 19.554 27.2292 20.002 27.7812 20.002C28.3333 20.002 28.7812 19.554 28.7812 19.002C28.7812 18.45 28.3333 18.002 27.7812 18.002ZM22.7812 18.002C22.2293 18.002 21.7812 18.45 21.7812 19.002C21.7812 19.554 22.2293 20.002 22.7812 20.002C23.3333 20.002 23.7812 19.554 23.7812 19.002C23.7812 18.45 23.3333 18.002 22.7812 18.002ZM32.7812 18.002C32.2292 18.002 31.7812 18.45 31.7812 19.002C31.7812 19.554 32.2292 20.002 32.7812 20.002C33.3333 20.002 33.7812 19.554 33.7812 19.002C33.7812 18.45 33.3333 18.002 32.7812 18.002ZM27.7812 22.002C27.2292 22.002 26.7812 22.45 26.7812 23.002C26.7812 23.554 27.2292 24.002 27.7812 24.002C28.3333 24.002 28.7812 23.554 28.7812 23.002C28.7812 22.45 28.3333 22.002 27.7812 22.002ZM32.7812 22.002C32.2292 22.002 31.7812 22.45 31.7812 23.002C31.7812 23.554 32.2292 24.002 32.7812 24.002C33.3333 24.002 33.7812 23.554 33.7812 23.002C33.7812 22.45 33.3333 22.002 32.7812 22.002ZM22.7812 22.002C22.2293 22.002 21.7812 22.45 21.7812 23.002C21.7812 23.554 22.2293 24.002 22.7812 24.002C23.3333 24.002 23.7812 23.554 23.7812 23.002C23.7812 22.45 23.3333 22.002 22.7812 22.002ZM27.7812 26.002C27.2292 26.002 26.7812 26.45 26.7812 27.002C26.7812 27.554 27.2292 28.002 27.7812 28.002C28.3333 28.002 28.7812 27.554 28.7812 27.002C28.7812 26.45 28.3333 26.002 27.7812 26.002ZM32.7812 26.002C32.2292 26.002 31.7812 26.45 31.7812 27.002C31.7812 27.554 32.2292 28.002 32.7812 28.002C33.3333 28.002 33.7812 27.554 33.7812 27.002C33.7812 26.45 33.3333 26.002 32.7812 26.002ZM22.7812 26.002C22.2293 26.002 21.7812 26.45 21.7812 27.002C21.7812 27.554 22.2293 28.002 22.7812 28.002C23.3333 28.002 23.7812 27.554 23.7812 27.002C23.7812 26.45 23.3333 26.002 22.7812 26.002ZM27.7812 30.002C27.2292 30.002 26.7812 30.45 26.7812 31.002C26.7812 31.554 27.2292 32.002 27.7812 32.002C28.3333 32.002 28.7812 31.554 28.7812 31.002C28.7812 30.45 28.3333 30.002 27.7812 30.002ZM32.7812 30.002C32.2292 30.002 31.7812 30.45 31.7812 31.002C31.7812 31.554 32.2292 32.002 32.7812 32.002C33.3333 32.002 33.7812 31.554 33.7812 31.002C33.7812 30.45 33.3333 30.002 32.7812 30.002ZM22.7812 30.002C22.2293 30.002 21.7812 30.45 21.7812 31.002C21.7812 31.554 22.2293 32.002 22.7812 32.002C23.3333 32.002 23.7812 31.554 23.7812 31.002C23.7812 30.45 23.3333 30.002 22.7812 30.002ZM27.7812 34.002C27.2292 34.002 26.7812 34.45 26.7812 35.002C26.7812 35.554 27.2292 36.002 27.7812 36.002C28.3333 36.002 28.7812 35.554 28.7812 35.002C28.7812 34.45 28.3333 34.002 27.7812 34.002ZM32.7812 34.002C32.2292 34.002 31.7812 34.45 31.7812 35.002C31.7812 35.554 32.2292 36.002 32.7812 36.002C33.3333 36.002 33.7812 35.554 33.7812 35.002C33.7812 34.45 33.3333 34.002 32.7812 34.002ZM22.7812 34.002C22.2293 34.002 21.7812 34.45 21.7812 35.002C21.7812 35.554 22.2293 36.002 22.7812 36.002C23.3333 36.002 23.7812 35.554 23.7812 35.002C23.7812 34.45 23.3333 34.002 22.7812 34.002Z" fill="#3C3C3C"></path> <path d="M32.7812 18.002C32.2292 18.002 31.7812 18.45 31.7812 19.002C31.7812 19.554 32.2292 20.002 32.7812 20.002C33.3333 20.002 33.7812 19.554 33.7812 19.002C33.7812 18.45 33.3333 18.002 32.7812 18.002ZM27.7812 18.002C27.2293 18.002 26.7812 18.45 26.7812 19.002C26.7812 19.554 27.2293 20.002 27.7812 20.002C28.3333 20.002 28.7812 19.554 28.7812 19.002C28.7812 18.45 28.3333 18.002 27.7812 18.002ZM32.7812 22.002C32.2292 22.002 31.7812 22.45 31.7812 23.002C31.7812 23.554 32.2292 24.002 32.7812 24.002C33.3333 24.002 33.7812 23.554 33.7812 23.002C33.7812 22.45 33.3333 22.002 32.7812 22.002ZM27.7812 22.002C27.2293 22.002 26.7812 22.45 26.7812 23.002C26.7812 23.554 27.2293 24.002 27.7812 24.002C28.3333 24.002 28.7812 23.554 28.7812 23.002C28.7812 22.45 28.3333 22.002 27.7812 22.002ZM32.7812 26.002C32.2292 26.002 31.7812 26.45 31.7812 27.002C31.7812 27.554 32.2292 28.002 32.7812 28.002C33.3333 28.002 33.7812 27.554 33.7812 27.002C33.7812 26.45 33.3333 26.002 32.7812 26.002ZM27.7812 26.002C27.2293 26.002 26.7812 26.45 26.7812 27.002C26.7812 27.554 27.2293 28.002 27.7812 28.002C28.3333 28.002 28.7812 27.554 28.7812 27.002C28.7812 26.45 28.3333 26.002 27.7812 26.002ZM32.7812 30.002C32.2292 30.002 31.7812 30.45 31.7812 31.002C31.7812 31.554 32.2292 32.002 32.7812 32.002C33.3333 32.002 33.7812 31.554 33.7812 31.002C33.7812 30.45 33.3333 30.002 32.7812 30.002ZM27.7812 30.002C27.2293 30.002 26.7812 30.45 26.7812 31.002C26.7812 31.554 27.2293 32.002 27.7812 32.002C28.3333 32.002 28.7812 31.554 28.7812 31.002C28.7812 30.45 28.3333 30.002 27.7812 30.002ZM32.7812 34.002C32.2292 34.002 31.7812 34.45 31.7812 35.002C31.7812 35.554 32.2292 36.002 32.7812 36.002C33.3333 36.002 33.7812 35.554 33.7812 35.002C33.7812 34.45 33.3333 34.002 32.7812 34.002ZM27.7812 34.002C27.2293 34.002 26.7812 34.45 26.7812 35.002C26.7812 35.554 27.2293 36.002 27.7812 36.002C28.3333 36.002 28.7812 35.554 28.7812 35.002C28.7812 34.45 28.3333 34.002 27.7812 34.002Z"></path>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 2 </span>
                                        </div>

                                        <!-- sq ft count -->
                                        <div class="flex justify-start items-center">

                                            <svg class="fill-black size-[15px]" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 297 297" xml:space="preserve">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <g>
                                                        <path d="M285.491,1.703c-4.467-1.284-9.238,0.551-11.699,4.49L101.994,281.069c-2.006,3.209-2.112,7.255-0.276,10.565 c1.834,3.311,5.321,5.365,9.105,5.365h171.798c5.75,0,10.412-4.662,10.412-10.412V11.712 C293.033,7.067,289.956,2.983,285.491,1.703z M272.209,276.176h-13.535v-6.768c0-5.75-4.662-10.412-10.412-10.412 s-10.412,4.662-10.412,10.412v6.768h-24.988v-6.768c0-5.75-4.662-10.412-10.412-10.412s-10.412,4.662-10.412,10.412v6.768h-24.989 v-6.768c0-5.75-4.661-10.412-10.411-10.412s-10.412,4.662-10.412,10.412v6.768H129.61L272.209,48.017V276.176z"></path>
                                                        <path d="M193.635,243.53h40.535c5.75,0,10.412-4.661,10.412-10.411V168.26c0-4.645-3.077-8.729-7.542-10.009 c-4.469-1.284-9.237,0.552-11.7,4.49l-40.535,64.859c-2.005,3.21-2.112,7.255-0.276,10.565 C186.362,241.477,189.849,243.53,193.635,243.53z M223.758,204.565v18.142H212.42L223.758,204.565z"></path>
                                                        <path d="M217.984,44.632c3.024-4.89,1.517-11.303-3.37-14.331l-46.387-28.74c-2.349-1.453-5.18-1.916-7.866-1.285 c-2.689,0.633-5.018,2.308-6.472,4.656l-4.371,7.063c-20.907-10.192-44.484-12.863-67.43-7.462 c-26.352,6.201-48.715,22.299-62.967,45.328C-8.446,94.412,2.747,152.213,43.364,183.531l-4.377,7.074 c-3.024,4.888-1.517,11.302,3.369,14.33l46.386,28.745c1.66,1.029,3.562,1.563,5.484,1.563c0.796,0,1.596-0.092,2.384-0.277 c2.689-0.632,5.017-2.307,6.471-4.656L217.984,44.632z M36.83,60.82c11.324-18.299,29.092-31.09,50.03-36.018 c17.521-4.122,35.5-2.303,51.618,5.033l-16.061,25.953c-9.116-3.315-19.003-3.928-28.668-1.651 C80.637,57.222,69.512,65.231,62.418,76.69c-12.755,20.615-8.822,46.932,8.05,63.046L54.41,165.684 C23.526,140.51,15.311,95.594,36.83,60.82z M81.853,121.338c-7.299-9.497-8.393-22.919-1.727-33.689 c4.163-6.728,10.695-11.431,18.393-13.241c4.179-0.984,8.43-1.043,12.52-0.23L81.853,121.338z M90.854,210.492l-28.684-17.775 L166.116,24.749l28.685,17.773l-8.193,13.241l-6.606-4.094c-4.889-3.03-11.306-1.522-14.334,3.365 c-3.029,4.888-1.523,11.306,3.365,14.335l6.617,4.101l-14.576,23.553l-6.607-4.093c-4.89-3.027-11.307-1.521-14.335,3.367 c-3.028,4.888-1.522,11.306,3.367,14.335l6.616,4.1l-14.574,23.553l-6.607-4.094c-4.889-3.026-11.307-1.52-14.335,3.368 c-3.027,4.889-1.521,11.306,3.368,14.334l6.616,4.1l-14.576,23.552l-6.604-4.094c-4.889-3.03-11.307-1.523-14.336,3.364 c-3.029,4.888-1.523,11.306,3.364,14.336l6.617,4.102L90.854,210.492z"></path>
                                                    </g>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 1560 Sq Ft </span>

                                        </div>

                                    </div>

                                </div>
                                <div>

                                    <!-- Property card on sale -->
                                    <div class="mb-3">
                                        <div class="mb-1 text-[13px] text-blue-900"> On sale </div>
                                        <div class="text-[18px]"> $540,000 </div>
                                    </div>

                                    <!-- Property card on author -->
                                    <div>
                                        <div class="text-[13px] text-blue-900"> By </div>
                                        <div class="text-[18px]"> Maria Villalobos </div>
                                    </div>

                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Property card -->
                    <div class="decoration-0 w-full">
                        <div class="mb-5 border flex flex-wrap duration-500 hover:shadow-xl group">
                            <div class="w-full sm:w-1/2">

                                <div class="relative">

                                    <!-- Property card image -->
                                    <img :src="'/images/properties-5.jpg'" class="object-cover bg-cover w-full h-[250px]" alt="properties"/>

                                    <div class="absolute bottom-0 start-0 top-0 end-0 p-3 flex justify-center items-center gap-x-3 bg-blue-200 bg-opacity-45 scale-0 duration-500 group-hover:scale-100">
                                        <button type="button" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill="#ffffff" d="M16.5562 12.9062L16.1007 13.359C16.1007 13.359 15.0181 14.4355 12.0631 11.4972C9.10812 8.55901 10.1907 7.48257 10.1907 7.48257L10.4775 7.19738C11.1841 6.49484 11.2507 5.36691 10.6342 4.54348L9.37326 2.85908C8.61028 1.83992 7.13596 1.70529 6.26145 2.57483L4.69185 4.13552C4.25823 4.56668 3.96765 5.12559 4.00289 5.74561C4.09304 7.33182 4.81071 10.7447 8.81536 14.7266C13.0621 18.9492 17.0468 19.117 18.6763 18.9651C19.1917 18.9171 19.6399 18.6546 20.0011 18.2954L21.4217 16.883C22.3806 15.9295 22.1102 14.2949 20.8833 13.628L18.9728 12.5894C18.1672 12.1515 17.1858 12.2801 16.5562 12.9062Z"></path>
                                                </g>
                                            </svg>
                                        </button>
                                        <button type="button" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="stroke-white duration-500 group-hover/item:stroke-blue-600" d="M4 7.00005L10.2 11.65C11.2667 12.45 12.7333 12.45 13.8 11.65L20 7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                    <rect class="stroke-white duration-500 group-hover/item:stroke-blue-600" x="3" y="5" width="18" height="14" rx="2" stroke-width="2" stroke-linecap="round"></rect>
                                                </g>
                                            </svg>
                                        </button>
                                        <router-link :to="{name: 'PropertyDetails'}" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M11.9944 15.5C13.9274 15.5 15.4944 13.933 15.4944 12C15.4944 10.067 13.9274 8.5 11.9944 8.5C10.0614 8.5 8.49439 10.067 8.49439 12C8.49439 13.933 10.0614 15.5 11.9944 15.5ZM11.9944 13.4944C11.1691 13.4944 10.5 12.8253 10.5 12C10.5 11.1747 11.1691 10.5056 11.9944 10.5056C12.8197 10.5056 13.4888 11.1747 13.4888 12C13.4888 12.8253 12.8197 13.4944 11.9944 13.4944Z"></path>
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M12 5C7.18879 5 3.9167 7.60905 2.1893 9.47978C0.857392 10.9222 0.857393 13.0778 2.1893 14.5202C3.9167 16.391 7.18879 19 12 19C16.8112 19 20.0833 16.391 21.8107 14.5202C23.1426 13.0778 23.1426 10.9222 21.8107 9.47978C20.0833 7.60905 16.8112 5 12 5ZM3.65868 10.8366C5.18832 9.18002 7.9669 7 12 7C16.0331 7 18.8117 9.18002 20.3413 10.8366C20.9657 11.5128 20.9657 12.4872 20.3413 13.1634C18.8117 14.82 16.0331 17 12 17C7.9669 17 5.18832 14.82 3.65868 13.1634C3.03426 12.4872 3.03426 11.5128 3.65868 10.8366Z"></path>
                                                </g>
                                            </svg>
                                        </router-link>
                                    </div>

                                </div>

                            </div>
                            <div class="w-full sm:w-1/2 p-3 py-3 flex justify-between flex-col">
                                <div class="mb-3">
                                    <div class="mb-3 text-[21px]">

                                        <!-- Property card title -->
                                        <div class="truncate-to-1-line">
                                            Modern Apartment
                                        </div>
                                    </div>
                                    <div class="text-[13px]">

                                        <!-- Property card description -->
                                        <div class="text-gray-500">
                                            4936 N Broadway St, Chicago, IL 60640, USA
                                        </div>

                                    </div>

                                    <div class="flex justify-start items-center text-[15px] gap-x-3 mt-3">

                                        <!-- bed count -->
                                        <div class="flex justify-start items-center">

                                            <svg class="fill-black size-[15px]" viewBox="0 0 50 50" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path d="M5 10C3.355469 10 2 11.355469 2 13L2 28.1875C2.003906 28.25 2.015625 28.3125 2.03125 28.375C2.03125 28.386719 2.03125 28.394531 2.03125 28.40625C1.582031 29.113281 1.214844 29.867188 0.9375 30.6875C0.316406 32.519531 0.0507813 34.621094 0 37L0 38C0 38.03125 0 38.0625 0 38.09375L0 50L7 50L7 46C7 45.167969 7.203125 44.734375 7.46875 44.46875C7.734375 44.203125 8.167969 44 9 44L41 44C41.832031 44 42.265625 44.203125 42.53125 44.46875C42.796875 44.734375 43 45.167969 43 46L43 50L50 50L50 38.15625C50.003906 38.105469 50.003906 38.050781 50 38C50 37.65625 50.007813 37.332031 50 37C49.949219 34.621094 49.683594 32.519531 49.0625 30.6875C48.785156 29.875 48.414063 29.136719 47.96875 28.4375C47.988281 28.355469 48 28.273438 48 28.1875L48 13C48 11.355469 46.644531 10 45 10 Z M 5 12L45 12C45.5625 12 46 12.4375 46 13L46 26.15625C45.753906 25.949219 45.492188 25.75 45.21875 25.5625C44.550781 25.101563 43.824219 24.671875 43 24.3125L43 20C43 19.296875 42.539063 18.75 42.03125 18.40625C41.523438 18.0625 40.902344 17.824219 40.125 17.625C38.570313 17.226563 36.386719 17 33.5 17C30.613281 17 28.429688 17.226563 26.875 17.625C26.117188 17.820313 25.5 18.042969 25 18.375C24.5 18.042969 23.882813 17.820313 23.125 17.625C21.570313 17.226563 19.386719 17 16.5 17C13.613281 17 11.429688 17.226563 9.875 17.625C9.097656 17.824219 8.476563 18.0625 7.96875 18.40625C7.460938 18.75 7 19.296875 7 20L7 24.3125C6.175781 24.671875 5.449219 25.101563 4.78125 25.5625C4.507813 25.75 4.246094 25.949219 4 26.15625L4 13C4 12.4375 4.4375 12 5 12 Z M 16.5 19C19.28125 19 21.34375 19.234375 22.625 19.5625C23.265625 19.726563 23.707031 19.925781 23.90625 20.0625C23.988281 20.117188 23.992188 20.125 24 20.125L24 22C17.425781 22.042969 12.558594 22.535156 9 23.625L9 20.125C9.007813 20.125 9.011719 20.117188 9.09375 20.0625C9.292969 19.925781 9.734375 19.726563 10.375 19.5625C11.65625 19.234375 13.71875 19 16.5 19 Z M 33.5 19C36.28125 19 38.34375 19.234375 39.625 19.5625C40.265625 19.726563 40.707031 19.925781 40.90625 20.0625C40.988281 20.117188 40.992188 20.125 41 20.125L41 23.625C37.441406 22.535156 32.574219 22.042969 26 22L26 20.125C26.007813 20.125 26.011719 20.117188 26.09375 20.0625C26.292969 19.925781 26.734375 19.726563 27.375 19.5625C28.65625 19.234375 30.71875 19 33.5 19 Z M 24.8125 24C24.917969 24.015625 25.019531 24.015625 25.125 24C25.15625 24 25.1875 24 25.21875 24C35.226563 24.015625 41.007813 25.0625 44.09375 27.1875C45.648438 28.257813 46.589844 29.585938 47.1875 31.34375C47.707031 32.875 47.917969 34.761719 47.96875 37L2.03125 37C2.082031 34.761719 2.292969 32.875 2.8125 31.34375C3.410156 29.585938 4.351563 28.257813 5.90625 27.1875C8.992188 25.058594 14.785156 24.011719 24.8125 24 Z M 2 39L48 39L48 48L45 48L45 46C45 44.832031 44.703125 43.765625 43.96875 43.03125C43.234375 42.296875 42.167969 42 41 42L9 42C7.832031 42 6.765625 42.296875 6.03125 43.03125C5.296875 43.765625 5 44.832031 5 46L5 48L2 48Z"></path>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 2 </span>
                                        </div>

                                        <!-- bath count -->
                                        <div class="flex justify-start items-center">

                                            <svg viewBox="0 0 45 45" class="size-[15px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="stroke-black" d="M27.7812 11.002V9.002C27.7812 7.897 26.8863 7.002 25.7812 7.002H14.7812C13.6762 7.002 12.7812 7.897 12.7812 9.002V36.002" stroke-width="2"></path>
                                                    <path class="stroke-black" d="M33.7812 11.002H21.7812V15.002H33.7812V11.002Z" stroke-width="2" stroke-linejoin="round"></path>
                                                    <path class="stroke-black" d="M18.7812 15.002H36.7812" stroke-width="2" stroke-linejoin="round"></path>
                                                    <path class="fill-black" d="M27.7812 18.002C27.2292 18.002 26.7812 18.45 26.7812 19.002C26.7812 19.554 27.2292 20.002 27.7812 20.002C28.3333 20.002 28.7812 19.554 28.7812 19.002C28.7812 18.45 28.3333 18.002 27.7812 18.002ZM22.7812 18.002C22.2293 18.002 21.7812 18.45 21.7812 19.002C21.7812 19.554 22.2293 20.002 22.7812 20.002C23.3333 20.002 23.7812 19.554 23.7812 19.002C23.7812 18.45 23.3333 18.002 22.7812 18.002ZM32.7812 18.002C32.2292 18.002 31.7812 18.45 31.7812 19.002C31.7812 19.554 32.2292 20.002 32.7812 20.002C33.3333 20.002 33.7812 19.554 33.7812 19.002C33.7812 18.45 33.3333 18.002 32.7812 18.002ZM27.7812 22.002C27.2292 22.002 26.7812 22.45 26.7812 23.002C26.7812 23.554 27.2292 24.002 27.7812 24.002C28.3333 24.002 28.7812 23.554 28.7812 23.002C28.7812 22.45 28.3333 22.002 27.7812 22.002ZM32.7812 22.002C32.2292 22.002 31.7812 22.45 31.7812 23.002C31.7812 23.554 32.2292 24.002 32.7812 24.002C33.3333 24.002 33.7812 23.554 33.7812 23.002C33.7812 22.45 33.3333 22.002 32.7812 22.002ZM22.7812 22.002C22.2293 22.002 21.7812 22.45 21.7812 23.002C21.7812 23.554 22.2293 24.002 22.7812 24.002C23.3333 24.002 23.7812 23.554 23.7812 23.002C23.7812 22.45 23.3333 22.002 22.7812 22.002ZM27.7812 26.002C27.2292 26.002 26.7812 26.45 26.7812 27.002C26.7812 27.554 27.2292 28.002 27.7812 28.002C28.3333 28.002 28.7812 27.554 28.7812 27.002C28.7812 26.45 28.3333 26.002 27.7812 26.002ZM32.7812 26.002C32.2292 26.002 31.7812 26.45 31.7812 27.002C31.7812 27.554 32.2292 28.002 32.7812 28.002C33.3333 28.002 33.7812 27.554 33.7812 27.002C33.7812 26.45 33.3333 26.002 32.7812 26.002ZM22.7812 26.002C22.2293 26.002 21.7812 26.45 21.7812 27.002C21.7812 27.554 22.2293 28.002 22.7812 28.002C23.3333 28.002 23.7812 27.554 23.7812 27.002C23.7812 26.45 23.3333 26.002 22.7812 26.002ZM27.7812 30.002C27.2292 30.002 26.7812 30.45 26.7812 31.002C26.7812 31.554 27.2292 32.002 27.7812 32.002C28.3333 32.002 28.7812 31.554 28.7812 31.002C28.7812 30.45 28.3333 30.002 27.7812 30.002ZM32.7812 30.002C32.2292 30.002 31.7812 30.45 31.7812 31.002C31.7812 31.554 32.2292 32.002 32.7812 32.002C33.3333 32.002 33.7812 31.554 33.7812 31.002C33.7812 30.45 33.3333 30.002 32.7812 30.002ZM22.7812 30.002C22.2293 30.002 21.7812 30.45 21.7812 31.002C21.7812 31.554 22.2293 32.002 22.7812 32.002C23.3333 32.002 23.7812 31.554 23.7812 31.002C23.7812 30.45 23.3333 30.002 22.7812 30.002ZM27.7812 34.002C27.2292 34.002 26.7812 34.45 26.7812 35.002C26.7812 35.554 27.2292 36.002 27.7812 36.002C28.3333 36.002 28.7812 35.554 28.7812 35.002C28.7812 34.45 28.3333 34.002 27.7812 34.002ZM32.7812 34.002C32.2292 34.002 31.7812 34.45 31.7812 35.002C31.7812 35.554 32.2292 36.002 32.7812 36.002C33.3333 36.002 33.7812 35.554 33.7812 35.002C33.7812 34.45 33.3333 34.002 32.7812 34.002ZM22.7812 34.002C22.2293 34.002 21.7812 34.45 21.7812 35.002C21.7812 35.554 22.2293 36.002 22.7812 36.002C23.3333 36.002 23.7812 35.554 23.7812 35.002C23.7812 34.45 23.3333 34.002 22.7812 34.002Z" fill="#3C3C3C"></path> <path d="M32.7812 18.002C32.2292 18.002 31.7812 18.45 31.7812 19.002C31.7812 19.554 32.2292 20.002 32.7812 20.002C33.3333 20.002 33.7812 19.554 33.7812 19.002C33.7812 18.45 33.3333 18.002 32.7812 18.002ZM27.7812 18.002C27.2293 18.002 26.7812 18.45 26.7812 19.002C26.7812 19.554 27.2293 20.002 27.7812 20.002C28.3333 20.002 28.7812 19.554 28.7812 19.002C28.7812 18.45 28.3333 18.002 27.7812 18.002ZM32.7812 22.002C32.2292 22.002 31.7812 22.45 31.7812 23.002C31.7812 23.554 32.2292 24.002 32.7812 24.002C33.3333 24.002 33.7812 23.554 33.7812 23.002C33.7812 22.45 33.3333 22.002 32.7812 22.002ZM27.7812 22.002C27.2293 22.002 26.7812 22.45 26.7812 23.002C26.7812 23.554 27.2293 24.002 27.7812 24.002C28.3333 24.002 28.7812 23.554 28.7812 23.002C28.7812 22.45 28.3333 22.002 27.7812 22.002ZM32.7812 26.002C32.2292 26.002 31.7812 26.45 31.7812 27.002C31.7812 27.554 32.2292 28.002 32.7812 28.002C33.3333 28.002 33.7812 27.554 33.7812 27.002C33.7812 26.45 33.3333 26.002 32.7812 26.002ZM27.7812 26.002C27.2293 26.002 26.7812 26.45 26.7812 27.002C26.7812 27.554 27.2293 28.002 27.7812 28.002C28.3333 28.002 28.7812 27.554 28.7812 27.002C28.7812 26.45 28.3333 26.002 27.7812 26.002ZM32.7812 30.002C32.2292 30.002 31.7812 30.45 31.7812 31.002C31.7812 31.554 32.2292 32.002 32.7812 32.002C33.3333 32.002 33.7812 31.554 33.7812 31.002C33.7812 30.45 33.3333 30.002 32.7812 30.002ZM27.7812 30.002C27.2293 30.002 26.7812 30.45 26.7812 31.002C26.7812 31.554 27.2293 32.002 27.7812 32.002C28.3333 32.002 28.7812 31.554 28.7812 31.002C28.7812 30.45 28.3333 30.002 27.7812 30.002ZM32.7812 34.002C32.2292 34.002 31.7812 34.45 31.7812 35.002C31.7812 35.554 32.2292 36.002 32.7812 36.002C33.3333 36.002 33.7812 35.554 33.7812 35.002C33.7812 34.45 33.3333 34.002 32.7812 34.002ZM27.7812 34.002C27.2293 34.002 26.7812 34.45 26.7812 35.002C26.7812 35.554 27.2293 36.002 27.7812 36.002C28.3333 36.002 28.7812 35.554 28.7812 35.002C28.7812 34.45 28.3333 34.002 27.7812 34.002Z"></path>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 2 </span>
                                        </div>

                                        <!-- sq ft count -->
                                        <div class="flex justify-start items-center">

                                            <svg class="fill-black size-[15px]" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 297 297" xml:space="preserve">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <g>
                                                        <path d="M285.491,1.703c-4.467-1.284-9.238,0.551-11.699,4.49L101.994,281.069c-2.006,3.209-2.112,7.255-0.276,10.565 c1.834,3.311,5.321,5.365,9.105,5.365h171.798c5.75,0,10.412-4.662,10.412-10.412V11.712 C293.033,7.067,289.956,2.983,285.491,1.703z M272.209,276.176h-13.535v-6.768c0-5.75-4.662-10.412-10.412-10.412 s-10.412,4.662-10.412,10.412v6.768h-24.988v-6.768c0-5.75-4.662-10.412-10.412-10.412s-10.412,4.662-10.412,10.412v6.768h-24.989 v-6.768c0-5.75-4.661-10.412-10.411-10.412s-10.412,4.662-10.412,10.412v6.768H129.61L272.209,48.017V276.176z"></path>
                                                        <path d="M193.635,243.53h40.535c5.75,0,10.412-4.661,10.412-10.411V168.26c0-4.645-3.077-8.729-7.542-10.009 c-4.469-1.284-9.237,0.552-11.7,4.49l-40.535,64.859c-2.005,3.21-2.112,7.255-0.276,10.565 C186.362,241.477,189.849,243.53,193.635,243.53z M223.758,204.565v18.142H212.42L223.758,204.565z"></path>
                                                        <path d="M217.984,44.632c3.024-4.89,1.517-11.303-3.37-14.331l-46.387-28.74c-2.349-1.453-5.18-1.916-7.866-1.285 c-2.689,0.633-5.018,2.308-6.472,4.656l-4.371,7.063c-20.907-10.192-44.484-12.863-67.43-7.462 c-26.352,6.201-48.715,22.299-62.967,45.328C-8.446,94.412,2.747,152.213,43.364,183.531l-4.377,7.074 c-3.024,4.888-1.517,11.302,3.369,14.33l46.386,28.745c1.66,1.029,3.562,1.563,5.484,1.563c0.796,0,1.596-0.092,2.384-0.277 c2.689-0.632,5.017-2.307,6.471-4.656L217.984,44.632z M36.83,60.82c11.324-18.299,29.092-31.09,50.03-36.018 c17.521-4.122,35.5-2.303,51.618,5.033l-16.061,25.953c-9.116-3.315-19.003-3.928-28.668-1.651 C80.637,57.222,69.512,65.231,62.418,76.69c-12.755,20.615-8.822,46.932,8.05,63.046L54.41,165.684 C23.526,140.51,15.311,95.594,36.83,60.82z M81.853,121.338c-7.299-9.497-8.393-22.919-1.727-33.689 c4.163-6.728,10.695-11.431,18.393-13.241c4.179-0.984,8.43-1.043,12.52-0.23L81.853,121.338z M90.854,210.492l-28.684-17.775 L166.116,24.749l28.685,17.773l-8.193,13.241l-6.606-4.094c-4.889-3.03-11.306-1.522-14.334,3.365 c-3.029,4.888-1.523,11.306,3.365,14.335l6.617,4.101l-14.576,23.553l-6.607-4.093c-4.89-3.027-11.307-1.521-14.335,3.367 c-3.028,4.888-1.522,11.306,3.367,14.335l6.616,4.1l-14.574,23.553l-6.607-4.094c-4.889-3.026-11.307-1.52-14.335,3.368 c-3.027,4.889-1.521,11.306,3.368,14.334l6.616,4.1l-14.576,23.552l-6.604-4.094c-4.889-3.03-11.307-1.523-14.336,3.364 c-3.029,4.888-1.523,11.306,3.364,14.336l6.617,4.102L90.854,210.492z"></path>
                                                    </g>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 1560 Sq Ft </span>

                                        </div>

                                    </div>

                                </div>
                                <div>

                                    <!-- Property card on sale -->
                                    <div class="mb-3">
                                        <div class="mb-1 text-[13px] text-blue-900"> On sale </div>
                                        <div class="text-[18px]"> $540,000 </div>
                                    </div>

                                    <!-- Property card on author -->
                                    <div>
                                        <div class="text-[13px] text-blue-900"> By </div>
                                        <div class="text-[18px]"> Maria Villalobos </div>
                                    </div>

                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Property card -->
                    <div class="decoration-0 w-full">
                        <div class="mb-5 border flex flex-wrap duration-500 hover:shadow-xl group">
                            <div class="w-full sm:w-1/2">

                                <div class="relative">

                                    <!-- Property card image -->
                                    <img :src="'/images/properties-6.jpg'" class="object-cover bg-cover w-full h-[250px]" alt="properties"/>

                                    <div class="absolute bottom-0 start-0 top-0 end-0 p-3 flex justify-center items-center gap-x-3 bg-blue-200 bg-opacity-45 scale-0 duration-500 group-hover:scale-100">
                                        <button type="button" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill="#ffffff" d="M16.5562 12.9062L16.1007 13.359C16.1007 13.359 15.0181 14.4355 12.0631 11.4972C9.10812 8.55901 10.1907 7.48257 10.1907 7.48257L10.4775 7.19738C11.1841 6.49484 11.2507 5.36691 10.6342 4.54348L9.37326 2.85908C8.61028 1.83992 7.13596 1.70529 6.26145 2.57483L4.69185 4.13552C4.25823 4.56668 3.96765 5.12559 4.00289 5.74561C4.09304 7.33182 4.81071 10.7447 8.81536 14.7266C13.0621 18.9492 17.0468 19.117 18.6763 18.9651C19.1917 18.9171 19.6399 18.6546 20.0011 18.2954L21.4217 16.883C22.3806 15.9295 22.1102 14.2949 20.8833 13.628L18.9728 12.5894C18.1672 12.1515 17.1858 12.2801 16.5562 12.9062Z"></path>
                                                </g>
                                            </svg>
                                        </button>
                                        <button type="button" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="stroke-white duration-500 group-hover/item:stroke-blue-600" d="M4 7.00005L10.2 11.65C11.2667 12.45 12.7333 12.45 13.8 11.65L20 7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                                    <rect class="stroke-white duration-500 group-hover/item:stroke-blue-600" x="3" y="5" width="18" height="14" rx="2" stroke-width="2" stroke-linecap="round"></rect>
                                                </g>
                                            </svg>
                                        </button>
                                        <router-link :to="{name: 'PropertyDetails'}" class="size-[40px] rounded outline-0 bg-blue-600 border border-blue-600 flex justify-center items-center hover:bg-transparent duration-500 group/item">
                                            <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M11.9944 15.5C13.9274 15.5 15.4944 13.933 15.4944 12C15.4944 10.067 13.9274 8.5 11.9944 8.5C10.0614 8.5 8.49439 10.067 8.49439 12C8.49439 13.933 10.0614 15.5 11.9944 15.5ZM11.9944 13.4944C11.1691 13.4944 10.5 12.8253 10.5 12C10.5 11.1747 11.1691 10.5056 11.9944 10.5056C12.8197 10.5056 13.4888 11.1747 13.4888 12C13.4888 12.8253 12.8197 13.4944 11.9944 13.4944Z"></path>
                                                    <path class="fill-white duration-500 group-hover/item:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M12 5C7.18879 5 3.9167 7.60905 2.1893 9.47978C0.857392 10.9222 0.857393 13.0778 2.1893 14.5202C3.9167 16.391 7.18879 19 12 19C16.8112 19 20.0833 16.391 21.8107 14.5202C23.1426 13.0778 23.1426 10.9222 21.8107 9.47978C20.0833 7.60905 16.8112 5 12 5ZM3.65868 10.8366C5.18832 9.18002 7.9669 7 12 7C16.0331 7 18.8117 9.18002 20.3413 10.8366C20.9657 11.5128 20.9657 12.4872 20.3413 13.1634C18.8117 14.82 16.0331 17 12 17C7.9669 17 5.18832 14.82 3.65868 13.1634C3.03426 12.4872 3.03426 11.5128 3.65868 10.8366Z"></path>
                                                </g>
                                            </svg>
                                        </router-link>
                                    </div>

                                </div>

                            </div>
                            <div class="w-full sm:w-1/2 p-3 py-3 flex justify-between flex-col">
                                <div class="mb-3">
                                    <div class="mb-3 text-[21px]">

                                        <!-- Property card title -->
                                        <div class="truncate-to-1-line">
                                            Modern Apartment
                                        </div>
                                    </div>
                                    <div class="text-[13px]">

                                        <!-- Property card description -->
                                        <div class="text-gray-500">
                                            4936 N Broadway St, Chicago, IL 60640, USA
                                        </div>

                                    </div>

                                    <div class="flex justify-start items-center text-[15px] gap-x-3 mt-3">

                                        <!-- bed count -->
                                        <div class="flex justify-start items-center">

                                            <svg class="fill-black size-[15px]" viewBox="0 0 50 50" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path d="M5 10C3.355469 10 2 11.355469 2 13L2 28.1875C2.003906 28.25 2.015625 28.3125 2.03125 28.375C2.03125 28.386719 2.03125 28.394531 2.03125 28.40625C1.582031 29.113281 1.214844 29.867188 0.9375 30.6875C0.316406 32.519531 0.0507813 34.621094 0 37L0 38C0 38.03125 0 38.0625 0 38.09375L0 50L7 50L7 46C7 45.167969 7.203125 44.734375 7.46875 44.46875C7.734375 44.203125 8.167969 44 9 44L41 44C41.832031 44 42.265625 44.203125 42.53125 44.46875C42.796875 44.734375 43 45.167969 43 46L43 50L50 50L50 38.15625C50.003906 38.105469 50.003906 38.050781 50 38C50 37.65625 50.007813 37.332031 50 37C49.949219 34.621094 49.683594 32.519531 49.0625 30.6875C48.785156 29.875 48.414063 29.136719 47.96875 28.4375C47.988281 28.355469 48 28.273438 48 28.1875L48 13C48 11.355469 46.644531 10 45 10 Z M 5 12L45 12C45.5625 12 46 12.4375 46 13L46 26.15625C45.753906 25.949219 45.492188 25.75 45.21875 25.5625C44.550781 25.101563 43.824219 24.671875 43 24.3125L43 20C43 19.296875 42.539063 18.75 42.03125 18.40625C41.523438 18.0625 40.902344 17.824219 40.125 17.625C38.570313 17.226563 36.386719 17 33.5 17C30.613281 17 28.429688 17.226563 26.875 17.625C26.117188 17.820313 25.5 18.042969 25 18.375C24.5 18.042969 23.882813 17.820313 23.125 17.625C21.570313 17.226563 19.386719 17 16.5 17C13.613281 17 11.429688 17.226563 9.875 17.625C9.097656 17.824219 8.476563 18.0625 7.96875 18.40625C7.460938 18.75 7 19.296875 7 20L7 24.3125C6.175781 24.671875 5.449219 25.101563 4.78125 25.5625C4.507813 25.75 4.246094 25.949219 4 26.15625L4 13C4 12.4375 4.4375 12 5 12 Z M 16.5 19C19.28125 19 21.34375 19.234375 22.625 19.5625C23.265625 19.726563 23.707031 19.925781 23.90625 20.0625C23.988281 20.117188 23.992188 20.125 24 20.125L24 22C17.425781 22.042969 12.558594 22.535156 9 23.625L9 20.125C9.007813 20.125 9.011719 20.117188 9.09375 20.0625C9.292969 19.925781 9.734375 19.726563 10.375 19.5625C11.65625 19.234375 13.71875 19 16.5 19 Z M 33.5 19C36.28125 19 38.34375 19.234375 39.625 19.5625C40.265625 19.726563 40.707031 19.925781 40.90625 20.0625C40.988281 20.117188 40.992188 20.125 41 20.125L41 23.625C37.441406 22.535156 32.574219 22.042969 26 22L26 20.125C26.007813 20.125 26.011719 20.117188 26.09375 20.0625C26.292969 19.925781 26.734375 19.726563 27.375 19.5625C28.65625 19.234375 30.71875 19 33.5 19 Z M 24.8125 24C24.917969 24.015625 25.019531 24.015625 25.125 24C25.15625 24 25.1875 24 25.21875 24C35.226563 24.015625 41.007813 25.0625 44.09375 27.1875C45.648438 28.257813 46.589844 29.585938 47.1875 31.34375C47.707031 32.875 47.917969 34.761719 47.96875 37L2.03125 37C2.082031 34.761719 2.292969 32.875 2.8125 31.34375C3.410156 29.585938 4.351563 28.257813 5.90625 27.1875C8.992188 25.058594 14.785156 24.011719 24.8125 24 Z M 2 39L48 39L48 48L45 48L45 46C45 44.832031 44.703125 43.765625 43.96875 43.03125C43.234375 42.296875 42.167969 42 41 42L9 42C7.832031 42 6.765625 42.296875 6.03125 43.03125C5.296875 43.765625 5 44.832031 5 46L5 48L2 48Z"></path>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 2 </span>
                                        </div>

                                        <!-- bath count -->
                                        <div class="flex justify-start items-center">

                                            <svg viewBox="0 0 45 45" class="size-[15px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <path class="stroke-black" d="M27.7812 11.002V9.002C27.7812 7.897 26.8863 7.002 25.7812 7.002H14.7812C13.6762 7.002 12.7812 7.897 12.7812 9.002V36.002" stroke-width="2"></path>
                                                    <path class="stroke-black" d="M33.7812 11.002H21.7812V15.002H33.7812V11.002Z" stroke-width="2" stroke-linejoin="round"></path>
                                                    <path class="stroke-black" d="M18.7812 15.002H36.7812" stroke-width="2" stroke-linejoin="round"></path>
                                                    <path class="fill-black" d="M27.7812 18.002C27.2292 18.002 26.7812 18.45 26.7812 19.002C26.7812 19.554 27.2292 20.002 27.7812 20.002C28.3333 20.002 28.7812 19.554 28.7812 19.002C28.7812 18.45 28.3333 18.002 27.7812 18.002ZM22.7812 18.002C22.2293 18.002 21.7812 18.45 21.7812 19.002C21.7812 19.554 22.2293 20.002 22.7812 20.002C23.3333 20.002 23.7812 19.554 23.7812 19.002C23.7812 18.45 23.3333 18.002 22.7812 18.002ZM32.7812 18.002C32.2292 18.002 31.7812 18.45 31.7812 19.002C31.7812 19.554 32.2292 20.002 32.7812 20.002C33.3333 20.002 33.7812 19.554 33.7812 19.002C33.7812 18.45 33.3333 18.002 32.7812 18.002ZM27.7812 22.002C27.2292 22.002 26.7812 22.45 26.7812 23.002C26.7812 23.554 27.2292 24.002 27.7812 24.002C28.3333 24.002 28.7812 23.554 28.7812 23.002C28.7812 22.45 28.3333 22.002 27.7812 22.002ZM32.7812 22.002C32.2292 22.002 31.7812 22.45 31.7812 23.002C31.7812 23.554 32.2292 24.002 32.7812 24.002C33.3333 24.002 33.7812 23.554 33.7812 23.002C33.7812 22.45 33.3333 22.002 32.7812 22.002ZM22.7812 22.002C22.2293 22.002 21.7812 22.45 21.7812 23.002C21.7812 23.554 22.2293 24.002 22.7812 24.002C23.3333 24.002 23.7812 23.554 23.7812 23.002C23.7812 22.45 23.3333 22.002 22.7812 22.002ZM27.7812 26.002C27.2292 26.002 26.7812 26.45 26.7812 27.002C26.7812 27.554 27.2292 28.002 27.7812 28.002C28.3333 28.002 28.7812 27.554 28.7812 27.002C28.7812 26.45 28.3333 26.002 27.7812 26.002ZM32.7812 26.002C32.2292 26.002 31.7812 26.45 31.7812 27.002C31.7812 27.554 32.2292 28.002 32.7812 28.002C33.3333 28.002 33.7812 27.554 33.7812 27.002C33.7812 26.45 33.3333 26.002 32.7812 26.002ZM22.7812 26.002C22.2293 26.002 21.7812 26.45 21.7812 27.002C21.7812 27.554 22.2293 28.002 22.7812 28.002C23.3333 28.002 23.7812 27.554 23.7812 27.002C23.7812 26.45 23.3333 26.002 22.7812 26.002ZM27.7812 30.002C27.2292 30.002 26.7812 30.45 26.7812 31.002C26.7812 31.554 27.2292 32.002 27.7812 32.002C28.3333 32.002 28.7812 31.554 28.7812 31.002C28.7812 30.45 28.3333 30.002 27.7812 30.002ZM32.7812 30.002C32.2292 30.002 31.7812 30.45 31.7812 31.002C31.7812 31.554 32.2292 32.002 32.7812 32.002C33.3333 32.002 33.7812 31.554 33.7812 31.002C33.7812 30.45 33.3333 30.002 32.7812 30.002ZM22.7812 30.002C22.2293 30.002 21.7812 30.45 21.7812 31.002C21.7812 31.554 22.2293 32.002 22.7812 32.002C23.3333 32.002 23.7812 31.554 23.7812 31.002C23.7812 30.45 23.3333 30.002 22.7812 30.002ZM27.7812 34.002C27.2292 34.002 26.7812 34.45 26.7812 35.002C26.7812 35.554 27.2292 36.002 27.7812 36.002C28.3333 36.002 28.7812 35.554 28.7812 35.002C28.7812 34.45 28.3333 34.002 27.7812 34.002ZM32.7812 34.002C32.2292 34.002 31.7812 34.45 31.7812 35.002C31.7812 35.554 32.2292 36.002 32.7812 36.002C33.3333 36.002 33.7812 35.554 33.7812 35.002C33.7812 34.45 33.3333 34.002 32.7812 34.002ZM22.7812 34.002C22.2293 34.002 21.7812 34.45 21.7812 35.002C21.7812 35.554 22.2293 36.002 22.7812 36.002C23.3333 36.002 23.7812 35.554 23.7812 35.002C23.7812 34.45 23.3333 34.002 22.7812 34.002Z" fill="#3C3C3C"></path> <path d="M32.7812 18.002C32.2292 18.002 31.7812 18.45 31.7812 19.002C31.7812 19.554 32.2292 20.002 32.7812 20.002C33.3333 20.002 33.7812 19.554 33.7812 19.002C33.7812 18.45 33.3333 18.002 32.7812 18.002ZM27.7812 18.002C27.2293 18.002 26.7812 18.45 26.7812 19.002C26.7812 19.554 27.2293 20.002 27.7812 20.002C28.3333 20.002 28.7812 19.554 28.7812 19.002C28.7812 18.45 28.3333 18.002 27.7812 18.002ZM32.7812 22.002C32.2292 22.002 31.7812 22.45 31.7812 23.002C31.7812 23.554 32.2292 24.002 32.7812 24.002C33.3333 24.002 33.7812 23.554 33.7812 23.002C33.7812 22.45 33.3333 22.002 32.7812 22.002ZM27.7812 22.002C27.2293 22.002 26.7812 22.45 26.7812 23.002C26.7812 23.554 27.2293 24.002 27.7812 24.002C28.3333 24.002 28.7812 23.554 28.7812 23.002C28.7812 22.45 28.3333 22.002 27.7812 22.002ZM32.7812 26.002C32.2292 26.002 31.7812 26.45 31.7812 27.002C31.7812 27.554 32.2292 28.002 32.7812 28.002C33.3333 28.002 33.7812 27.554 33.7812 27.002C33.7812 26.45 33.3333 26.002 32.7812 26.002ZM27.7812 26.002C27.2293 26.002 26.7812 26.45 26.7812 27.002C26.7812 27.554 27.2293 28.002 27.7812 28.002C28.3333 28.002 28.7812 27.554 28.7812 27.002C28.7812 26.45 28.3333 26.002 27.7812 26.002ZM32.7812 30.002C32.2292 30.002 31.7812 30.45 31.7812 31.002C31.7812 31.554 32.2292 32.002 32.7812 32.002C33.3333 32.002 33.7812 31.554 33.7812 31.002C33.7812 30.45 33.3333 30.002 32.7812 30.002ZM27.7812 30.002C27.2293 30.002 26.7812 30.45 26.7812 31.002C26.7812 31.554 27.2293 32.002 27.7812 32.002C28.3333 32.002 28.7812 31.554 28.7812 31.002C28.7812 30.45 28.3333 30.002 27.7812 30.002ZM32.7812 34.002C32.2292 34.002 31.7812 34.45 31.7812 35.002C31.7812 35.554 32.2292 36.002 32.7812 36.002C33.3333 36.002 33.7812 35.554 33.7812 35.002C33.7812 34.45 33.3333 34.002 32.7812 34.002ZM27.7812 34.002C27.2293 34.002 26.7812 34.45 26.7812 35.002C26.7812 35.554 27.2293 36.002 27.7812 36.002C28.3333 36.002 28.7812 35.554 28.7812 35.002C28.7812 34.45 28.3333 34.002 27.7812 34.002Z"></path>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 2 </span>
                                        </div>

                                        <!-- sq ft count -->
                                        <div class="flex justify-start items-center">

                                            <svg class="fill-black size-[15px]" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 297 297" xml:space="preserve">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <g>
                                                        <path d="M285.491,1.703c-4.467-1.284-9.238,0.551-11.699,4.49L101.994,281.069c-2.006,3.209-2.112,7.255-0.276,10.565 c1.834,3.311,5.321,5.365,9.105,5.365h171.798c5.75,0,10.412-4.662,10.412-10.412V11.712 C293.033,7.067,289.956,2.983,285.491,1.703z M272.209,276.176h-13.535v-6.768c0-5.75-4.662-10.412-10.412-10.412 s-10.412,4.662-10.412,10.412v6.768h-24.988v-6.768c0-5.75-4.662-10.412-10.412-10.412s-10.412,4.662-10.412,10.412v6.768h-24.989 v-6.768c0-5.75-4.661-10.412-10.411-10.412s-10.412,4.662-10.412,10.412v6.768H129.61L272.209,48.017V276.176z"></path>
                                                        <path d="M193.635,243.53h40.535c5.75,0,10.412-4.661,10.412-10.411V168.26c0-4.645-3.077-8.729-7.542-10.009 c-4.469-1.284-9.237,0.552-11.7,4.49l-40.535,64.859c-2.005,3.21-2.112,7.255-0.276,10.565 C186.362,241.477,189.849,243.53,193.635,243.53z M223.758,204.565v18.142H212.42L223.758,204.565z"></path>
                                                        <path d="M217.984,44.632c3.024-4.89,1.517-11.303-3.37-14.331l-46.387-28.74c-2.349-1.453-5.18-1.916-7.866-1.285 c-2.689,0.633-5.018,2.308-6.472,4.656l-4.371,7.063c-20.907-10.192-44.484-12.863-67.43-7.462 c-26.352,6.201-48.715,22.299-62.967,45.328C-8.446,94.412,2.747,152.213,43.364,183.531l-4.377,7.074 c-3.024,4.888-1.517,11.302,3.369,14.33l46.386,28.745c1.66,1.029,3.562,1.563,5.484,1.563c0.796,0,1.596-0.092,2.384-0.277 c2.689-0.632,5.017-2.307,6.471-4.656L217.984,44.632z M36.83,60.82c11.324-18.299,29.092-31.09,50.03-36.018 c17.521-4.122,35.5-2.303,51.618,5.033l-16.061,25.953c-9.116-3.315-19.003-3.928-28.668-1.651 C80.637,57.222,69.512,65.231,62.418,76.69c-12.755,20.615-8.822,46.932,8.05,63.046L54.41,165.684 C23.526,140.51,15.311,95.594,36.83,60.82z M81.853,121.338c-7.299-9.497-8.393-22.919-1.727-33.689 c4.163-6.728,10.695-11.431,18.393-13.241c4.179-0.984,8.43-1.043,12.52-0.23L81.853,121.338z M90.854,210.492l-28.684-17.775 L166.116,24.749l28.685,17.773l-8.193,13.241l-6.606-4.094c-4.889-3.03-11.306-1.522-14.334,3.365 c-3.029,4.888-1.523,11.306,3.365,14.335l6.617,4.101l-14.576,23.553l-6.607-4.093c-4.89-3.027-11.307-1.521-14.335,3.367 c-3.028,4.888-1.522,11.306,3.367,14.335l6.616,4.1l-14.574,23.553l-6.607-4.094c-4.889-3.026-11.307-1.52-14.335,3.368 c-3.027,4.889-1.521,11.306,3.368,14.334l6.616,4.1l-14.576,23.552l-6.604-4.094c-4.889-3.03-11.307-1.523-14.336,3.364 c-3.029,4.888-1.523,11.306,3.364,14.336l6.617,4.102L90.854,210.492z"></path>
                                                    </g>
                                                </g>
                                            </svg>

                                            <span class="ms-2"> 1560 Sq Ft </span>

                                        </div>

                                    </div>

                                </div>
                                <div>

                                    <!-- Property card on sale -->
                                    <div class="mb-3">
                                        <div class="mb-1 text-[13px] text-blue-900"> On sale </div>
                                        <div class="text-[18px]"> $540,000 </div>
                                    </div>

                                    <!-- Property card on author -->
                                    <div>
                                        <div class="text-[13px] text-blue-900"> By </div>
                                        <div class="text-[18px]"> Maria Villalobos </div>
                                    </div>

                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Pagination -->
                    <div class="my-10 flex justify-center items-center gap-x-2">
                        <a href="javascript:void(0)" class="size-[35px] text-[13px] text-gray-500 duration-500 hover:bg-blue-700 font-medium hover:text-white flex justify-center items-center rounded-full shadow-[0_0_3px_0_#888888]">
                            1
                        </a>
                        <a href="javascript:void(0)" class="size-[35px] text-[13px] text-gray-500 duration-500 hover:bg-blue-700 font-medium hover:text-white flex justify-center items-center rounded-full shadow-[0_0_3px_0_#888888]">
                            2
                        </a>
                        <a href="javascript:void(0)" class="size-[35px] text-[13px] text-gray-500 duration-500 hover:bg-blue-700 font-medium hover:text-white flex justify-center items-center rounded-full shadow-[0_0_3px_0_#888888]">
                            3
                        </a>
                        <a href="javascript:void(0)" class="size-[35px] text-[13px] text-gray-500 duration-500 hover:bg-blue-700 font-medium hover:text-white flex justify-center items-center rounded-full shadow-[0_0_3px_0_#888888]">
                            4
                        </a>
                        <a href="javascript:void(0)" class="size-[35px] text-[13px] text-gray-500 duration-500 hover:bg-blue-700 font-medium hover:text-white flex justify-center items-center rounded-full shadow-[0_0_3px_0_#888888]">
                            5
                        </a>
                    </div>

                </div>

                <div class="w-full lg:w-1/3 md:ps-5 order-1 mb-5 lg:order-2">

                    <!-- Search -->
                    <div class="mb-5">
                        <label for="keyword" hidden="hidden"> keyword </label>
                        <div class="relative w-full">
                            <input type="text" id="keyword" placeholder="Search here..." name="keyword" class="text-[13px] h-[42px] rounded-md w-full outline-0 border border-gray-300 focus-visible:border-blue-900 duration-500 ps-10 pe-3 py-2" required autocomplete="off">
                            <div class="absolute top-0 bottom-0 start-0 flex justify-center items-center ps-2">
                                <svg viewBox="0 0 24 24" class="size-[24px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                    <g id="SVGRepo_iconCarrier">
                                        <path class="stroke-gray-400" d="M11 6C13.7614 6 16 8.23858 16 11M16.6588 16.6549L21 21M19 11C19 15.4183 15.4183 19 11 19C6.58172 19 3 15.4183 3 11C3 6.58172 6.58172 3 11 3C15.4183 3 19 6.58172 19 11Z" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                                    </g>
                                </svg>
                            </div>
                        </div>
                    </div>

                    <!-- Property types -->
                    <div class="mb-5 text-[21px]">
                        Types of property
                    </div>

                    <!-- Property types list -->
                    <div class="text-[15px] ps-5">
                        <div class="my-3">
                            <a href="javascript:void(0)" class="decoration-0 text-black duration-500 hover:text-blue-600 group flex justify-start items-center">
                                <svg viewBox="0 0 24 24" class="size-[20px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                    <g id="SVGRepo_iconCarrier">
                                        <path class="fill-black duration-500 group-hover:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M8.5 17C8.5 17.4045 8.74364 17.7691 9.11732 17.9239C9.49099 18.0787 9.92111 17.9931 10.2071 17.7071L15.2071 12.7071C15.5976 12.3166 15.5976 11.6834 15.2071 11.2929L10.2071 6.29289C9.92111 6.00689 9.49099 5.92134 9.11732 6.07612C8.74364 6.2309 8.5 6.59554 8.5 7V17Z"></path>
                                    </g>
                                </svg>
                                <span class="ms-1">
                                    Commercial
                                </span>
                            </a>
                        </div>
                        <div class="ms-5">
                            <div class="mb-3">
                                <a href="javascript:void(0)" class="decoration-0 text-black duration-500 hover:text-blue-600 group flex justify-start items-center">
                                    <svg viewBox="0 0 24 24" class="size-[20px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                        <g id="SVGRepo_iconCarrier">
                                            <path class="fill-black duration-500 group-hover:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M8.5 17C8.5 17.4045 8.74364 17.7691 9.11732 17.9239C9.49099 18.0787 9.92111 17.9931 10.2071 17.7071L15.2071 12.7071C15.5976 12.3166 15.5976 11.6834 15.2071 11.2929L10.2071 6.29289C9.92111 6.00689 9.49099 5.92134 9.11732 6.07612C8.74364 6.2309 8.5 6.59554 8.5 7V17Z"></path>
                                        </g>
                                    </svg>
                                    <span class="ms-1">
                                        Office
                                    </span>
                                </a>
                            </div>
                            <div class="mb-3">
                                <a href="javascript:void(0)" class="decoration-0 text-black duration-500 hover:text-blue-600 group flex justify-start items-center">
                                    <svg viewBox="0 0 24 24" class="size-[20px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                        <g id="SVGRepo_iconCarrier">
                                            <path class="fill-black duration-500 group-hover:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M8.5 17C8.5 17.4045 8.74364 17.7691 9.11732 17.9239C9.49099 18.0787 9.92111 17.9931 10.2071 17.7071L15.2071 12.7071C15.5976 12.3166 15.5976 11.6834 15.2071 11.2929L10.2071 6.29289C9.92111 6.00689 9.49099 5.92134 9.11732 6.07612C8.74364 6.2309 8.5 6.59554 8.5 7V17Z"></path>
                                        </g>
                                    </svg>
                                    <span class="ms-1">
                                        Store
                                    </span>
                                </a>
                            </div>
                        </div>
                        <div class="my-3">
                            <a href="javascript:void(0)" class="decoration-0 text-black duration-500 hover:text-blue-600 group flex justify-start items-center">
                                <svg viewBox="0 0 24 24" class="size-[20px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                    <g id="SVGRepo_iconCarrier">
                                        <path class="fill-black duration-500 group-hover:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M8.5 17C8.5 17.4045 8.74364 17.7691 9.11732 17.9239C9.49099 18.0787 9.92111 17.9931 10.2071 17.7071L15.2071 12.7071C15.5976 12.3166 15.5976 11.6834 15.2071 11.2929L10.2071 6.29289C9.92111 6.00689 9.49099 5.92134 9.11732 6.07612C8.74364 6.2309 8.5 6.59554 8.5 7V17Z"></path>
                                    </g>
                                </svg>
                                <span class="ms-1">
                                    Residential
                                </span>
                            </a>
                        </div>
                        <div class="ms-5">
                            <div class="mb-3">
                                <a href="javascript:void(0)" class="decoration-0 text-black duration-500 hover:text-blue-600 group flex justify-start items-center">
                                    <svg viewBox="0 0 24 24" class="size-[20px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                        <g id="SVGRepo_iconCarrier">
                                            <path class="fill-black duration-500 group-hover:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M8.5 17C8.5 17.4045 8.74364 17.7691 9.11732 17.9239C9.49099 18.0787 9.92111 17.9931 10.2071 17.7071L15.2071 12.7071C15.5976 12.3166 15.5976 11.6834 15.2071 11.2929L10.2071 6.29289C9.92111 6.00689 9.49099 5.92134 9.11732 6.07612C8.74364 6.2309 8.5 6.59554 8.5 7V17Z"></path>
                                        </g>
                                    </svg>
                                    <span class="ms-1">
                                        Apartment
                                    </span>
                                </a>
                            </div>
                            <div class="mb-3">
                                <a href="javascript:void(0)" class="decoration-0 text-black duration-500 hover:text-blue-600 group flex justify-start items-center">
                                    <svg viewBox="0 0 24 24" class="size-[20px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                        <g id="SVGRepo_iconCarrier">
                                            <path class="fill-black duration-500 group-hover:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M8.5 17C8.5 17.4045 8.74364 17.7691 9.11732 17.9239C9.49099 18.0787 9.92111 17.9931 10.2071 17.7071L15.2071 12.7071C15.5976 12.3166 15.5976 11.6834 15.2071 11.2929L10.2071 6.29289C9.92111 6.00689 9.49099 5.92134 9.11732 6.07612C8.74364 6.2309 8.5 6.59554 8.5 7V17Z"></path>
                                        </g>
                                    </svg>
                                    <span class="ms-1">
                                        Condominium
                                    </span>
                                </a>
                            </div>
                            <div class="mb-3">
                                <a href="javascript:void(0)" class="decoration-0 text-black duration-500 hover:text-blue-600 group flex justify-start items-center">
                                    <svg viewBox="0 0 24 24" class="size-[20px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                        <g id="SVGRepo_iconCarrier">
                                            <path class="fill-black duration-500 group-hover:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M8.5 17C8.5 17.4045 8.74364 17.7691 9.11732 17.9239C9.49099 18.0787 9.92111 17.9931 10.2071 17.7071L15.2071 12.7071C15.5976 12.3166 15.5976 11.6834 15.2071 11.2929L10.2071 6.29289C9.92111 6.00689 9.49099 5.92134 9.11732 6.07612C8.74364 6.2309 8.5 6.59554 8.5 7V17Z"></path>
                                        </g>
                                    </svg>
                                    <span class="ms-1">
                                        Apartment Building
                                    </span>
                                </a>
                            </div>
                            <div class="mb-3">
                                <a href="javascript:void(0)" class="decoration-0 text-black duration-500 hover:text-blue-600 group flex justify-start items-center">
                                    <svg viewBox="0 0 24 24" class="size-[20px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                        <g id="SVGRepo_iconCarrier">
                                            <path class="fill-black duration-500 group-hover:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M8.5 17C8.5 17.4045 8.74364 17.7691 9.11732 17.9239C9.49099 18.0787 9.92111 17.9931 10.2071 17.7071L15.2071 12.7071C15.5976 12.3166 15.5976 11.6834 15.2071 11.2929L10.2071 6.29289C9.92111 6.00689 9.49099 5.92134 9.11732 6.07612C8.74364 6.2309 8.5 6.59554 8.5 7V17Z"></path>
                                        </g>
                                    </svg>
                                    <span class="ms-1">
                                        Single Family
                                    </span>
                                </a>
                            </div>
                            <div class="mb-3">
                                <a href="javascript:void(0)" class="decoration-0 text-black duration-500 hover:text-blue-600 group flex justify-start items-center">
                                    <svg viewBox="0 0 24 24" class="size-[20px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                        <g id="SVGRepo_iconCarrier">
                                            <path class="fill-black duration-500 group-hover:fill-blue-600" fill-rule="evenodd" clip-rule="evenodd" d="M8.5 17C8.5 17.4045 8.74364 17.7691 9.11732 17.9239C9.49099 18.0787 9.92111 17.9931 10.2071 17.7071L15.2071 12.7071C15.5976 12.3166 15.5976 11.6834 15.2071 11.2929L10.2071 6.29289C9.92111 6.00689 9.49099 5.92134 9.11732 6.07612C8.74364 6.2309 8.5 6.59554 8.5 7V17Z"></path>
                                        </g>
                                    </svg>
                                    <span class="ms-1">
                                        Villa
                                    </span>
                                </a>
                            </div>
                        </div>
                    </div>

                    <!-- Property status -->
                    <div class="my-5 text-[21px]">
                        Status of property
                    </div>

                    <!-- Property status list -->
                    <div class="flex flex-wrap justify-start items-start gap-1">
                        <a href="javascript:void(0)" class="decoration-0 bg-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg text-white text-[13px] whitespace-break-spaces">
                            Purchase
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 bg-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg text-white text-[13px] whitespace-break-spaces">
                            Rent
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 bg-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg text-white text-[13px] whitespace-break-spaces">
                            Sell
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 bg-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg text-white text-[13px] whitespace-break-spaces">
                            Miss
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 bg-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg text-white text-[13px] whitespace-break-spaces">
                            Evaluation
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 bg-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg text-white text-[13px] whitespace-break-spaces">
                            Mortgage
                        </a>
                    </div>

                    <!-- Location -->
                    <div class="my-5 text-[21px]">
                        Location
                    </div>

                    <!-- Location list -->
                    <div class="flex flex-wrap justify-start items-start gap-1">
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            United States
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Bangladesh
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Canada
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Brazil
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Argentina
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            United Kingdom
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Germany
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            France
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Italy
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Spain
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Russia
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            China
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Japan
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            South Korea
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            India
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Australia
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            New Zealand
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Mexico
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            South Africa
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Egypt
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Saudi Arabia
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Turkey
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Greece
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Sweden
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Norway
                        </a>
                        <a href="javascript:void(0)" class="decoration-0 border border-blue-500 text-blue-500 duration-500 hover:bg-blue-700 px-4 py-2 rounded-lg hover:text-white text-[13px] whitespace-break-spaces">
                            Netherlands
                        </a>
                    </div>

                </div>

            </div>

        </div>

    </section>

</template>

<script>

export default {
    data(){
        return {

        }
    },
    mounted() {

    },
    methods: {

    }
}

</script>
